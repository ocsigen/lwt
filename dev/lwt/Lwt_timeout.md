...
The timeout is not started until [`Lwt_timeout.start`](./#val-start) is called on it.
...
If the timeout has already been started, it is stopped, and restarted with its new duration. This is similar to how [`Lwt_timeout.start`](./#val-start) works on a timeout that has already been started.
...
The default behavior of `f exn`, set by `Lwt_timeout` on program startup, is to pass `exn` to `!`[`Lwt.async_exception_hook`](./Lwt.md#val-async_exception_hook). The default behavior of *that* is to terminate the process.
