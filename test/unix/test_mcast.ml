...
let child mcast_addr join fd =
  ...
  Lwt_unix.with_timeout 1. (fun () -> Lwt_unix.read fd buf 0 (Bytes.length buf))
  >>= fun n ->
  if debug then
...
let parent mcast_addr mcast_port set_loop fd =
  ...
  let addr =
    Lwt_unix.ADDR_INET (Unix.inet_addr_of_string mcast_addr, mcast_port)
  in
...
let test_mcast name join set_loop =
  test name
    ...
    (fun () ->
      ...
      in
      Lwt.finalize t (fun () ->
          Lwt.join [ Lwt_unix.close fd1; Lwt_unix.close fd2 ]))
