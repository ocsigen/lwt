...
module Establish_server = struct
  let with_client f =
    let local = local () in
    ...
    >>= fun server ->
    let client_finished =
      Lwt_io.with_connection local (fun (_, out_channel) ->
          Lwt_io.write out_channel "hello world" >>= fun () -> handler_finished)
    in

    client_finished >>= fun () -> Lwt_io.shutdown_server server
...
let suite =
  suite "lwt_io"
    [
      test "auto-flush" ~sequential:true (fun () ->
          let sent = ref [] in
          let oc =
            Lwt_io.make ~mode:Lwt_io.output (fun buf ofs len ->
      ...
      test "auto-flush in atomic" ~sequential:true (fun () ->
          let sent = ref [] in
          let oc =
            Lwt_io.make ~mode:Lwt_io.output (fun buf ofs len ->
                let bytes = Bytes.create len in
      ...
      test "establish_server_1: shutdown: client closes first"
        ...
        (* Note: this test is currently flaky on Linux with libev enabled, so we skip
          ...
          let handler =
            ...
            Lwt_io.close out_channel >>= fun () -> Lwt.return_true
          in

          let local = local () in

          let server =
            (Lwt_io.Versioned.establish_server_1 [@ocaml.warning "-3"]) local
              (fun channels -> Lwt.wakeup run_handler channels)
          in

          Lwt_io.with_connection local (fun _ -> Lwt.return_unit) >>= fun () ->
          Lwt.wakeup client_finished ();
          Lwt_io.shutdown_server server >>= fun () -> handler);
      (* Counterpart to establish_server: shutdown test. Confirms that shutdown is
         implemented correctly in open_connection. *)
      test "open_connection: shutdown: server closes first" (fun () ->
          ...
          Lwt_io.with_connection local (fun _ ->
              wait_for_server >>= fun () -> Lwt.return_true)
          >>= fun result ->
          Lwt_io.shutdown_server server >|= fun () -> result);
      test "establish_server: implicit close" (fun () ->
          ...
          let run =
            Establish_server.with_client (fun (in_channel, out_channel) ->
      ...
      test ~sequential:true "establish_server: implicit close on exception"
        (fun () ->
          ...
          with_async_exception_hook
            (function Exit -> exit_raised := true | _ -> ())
            run
          >>= fun () ->
      ...
      test "establish_server: explicit close" (fun () ->
          let open Establish_server in
          ...
          let run =
            Establish_server.with_client (fun (in_channel, out_channel) ->
                ...
                closed_explicitly :=
                  in_closed_in_handler && out_closed_in_handler)
          in

          run >|= fun () -> !closed_explicitly);
      test "with_connection" (fun () ->
          let open Establish_server in
          let in_channel' = ref Lwt_io.stdin in
          let out_channel' = ref Lwt_io.stdout in
      ...
      (* Makes the channel fail with EBADF on close. Tries to close the channel
         ...
         again. *)
      test "with_close_connection: no duplicate exceptions" (fun () ->
      ...
      test "with_temp_filename" (fun () ->
          ...
          Lwt.catch
            ...
            (fun exn ->
              if exn = Dummy_error then
                Lwt.return (not (Sys.file_exists !filename))
              else Lwt.return_false)
          >>= fun no_temps2 -> Lwt.return (no_temps1 && no_temps2));
      (* Verify that no exceptions are thrown if the function passed to
         with_temp_file closes the channel on its own. *)
      test "with_temp_filename close handle" (fun () ->
          let f (_, chan) =
            Lwt_io.write chan "test file content" >>= fun _ -> Lwt_io.close chan
          in
          Lwt_io.with_temp_file f >>= fun _ -> Lwt.return_true);
      test "create_temp_dir" (fun () ->
          let prefix = "temp_dir" in
          let suffix = "_foo" in
          Lwt_io.create_temp_dir ~parent:Filename.current_dir_name ~prefix
            ~suffix ()
          >>= fun path ->
      ...
      test "with_temp_dir" ~sequential:true (fun () ->
          Lwt_io.with_temp_dir ~parent:Filename.current_dir_name
            ~prefix:"temp_dir" (fun path ->
          ...
          >>= fun (path, directory_existed, had_files) ->
          let directory_removed = not (Sys.file_exists path) in

          Lwt.return (directory_existed && had_files && directory_removed));
      test "file_length on directory" (fun () ->
          Lwt.catch
            (fun () -> Lwt_io.file_length "." >>= fun _ -> Lwt.return false)
            (function
              | Unix.Unix_error (Unix.EISDIR, "file_length", ".") ->
                  Lwt.return true
              | exn -> Lwt.fail exn));
      test "input channel of_bytes initial position" (fun () ->
          let ichan =
            Lwt_io.of_bytes ~mode:Lwt_io.input @@ Lwt_bytes.of_string "abcd"
          in
          Lwt.return (Lwt_io.position ichan = 0L));
      test "input channel of_bytes position after read" (fun () ->
          let ichan =
            Lwt_io.of_bytes ~mode:Lwt_io.input @@ Lwt_bytes.of_string "abcd"
          in
          Lwt_io.read_char ichan >|= fun _ -> Lwt_io.position ichan = 1L);
      test "input channel of_bytes position after set_position" (fun () ->
          let ichan =
            Lwt_io.of_bytes ~mode:Lwt_io.input @@ Lwt_bytes.of_string "abcd"
          in
          Lwt_io.set_position ichan 2L >|= fun () -> Lwt_io.position ichan = 2L);
      test "output channel of_bytes initial position" (fun () ->
          let ochan =
            Lwt_io.of_bytes ~mode:Lwt_io.output @@ Lwt_bytes.create 4
          in
          Lwt.return (Lwt_io.position ochan = 0L));
      test "output channel of_bytes position after read" (fun () ->
          let ochan =
            Lwt_io.of_bytes ~mode:Lwt_io.output @@ Lwt_bytes.create 4
          in
          Lwt_io.write_char ochan 'a' >|= fun _ -> Lwt_io.position ochan = 1L);
      test "output channel of_bytes position after set_position" (fun () ->
          let ochan =
            Lwt_io.of_bytes ~mode:Lwt_io.output @@ Lwt_bytes.create 4
          in
          Lwt_io.set_position ochan 2L >|= fun _ -> Lwt_io.position ochan = 2L);
      test "NumberIO.LE.read_int" (fun () ->
          Lwt_bytes.of_string "\x01\x02\x03\x04"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_int
          >|= ( = ) 0x04030201);
      test "NumberIO.BE.read_int" (fun () ->
          Lwt_bytes.of_string "\x01\x02\x03\x04"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_int
          >|= ( = ) 0x01020304);
      test "NumberIO.LE.read_int16" (fun () ->
          Lwt_bytes.of_string "\x01\x02"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_int16
          >|= ( = ) 0x0201);
      test "NumberIO.BE.read_int16" (fun () ->
          Lwt_bytes.of_string "\x01\x02"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_int16
          >|= ( = ) 0x0102);
      test "NumberIO.LE.read_int16, negative" (fun () ->
          Lwt_bytes.of_string "\xfe\xff"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_int16
          >|= ( = ) (-2));
      test "NumberIO.BE.read_int16, negative" (fun () ->
          Lwt_bytes.of_string "\xff\xfe"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_int16
          >|= ( = ) (-2));
      test "NumberIO.LE.read_int32" (fun () ->
          Lwt_bytes.of_string "\x01\x02\x03\x04"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_int32
          >|= ( = ) 0x04030201l);
      test "NumberIO.BE.read_int32" (fun () ->
          Lwt_bytes.of_string "\x01\x02\x03\x04"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_int32
          >|= ( = ) 0x01020304l);
      test "NumberIO.LE.read_int64" (fun () ->
          Lwt_bytes.of_string "\x01\x02\x03\x04\x05\x06\x07\x08"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_int64
          >|= ( = ) 0x0807060504030201L);
      test "NumberIO.BE.read_int64" (fun () ->
          Lwt_bytes.of_string "\x01\x02\x03\x04\x05\x06\x07\x08"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_int64
          >|= ( = ) 0x0102030405060708L);
      test "NumberIO.LE.read_float32" (fun () ->
          Lwt_bytes.of_string "\x80\x01\x81\x47"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_float32
          >|= fun n -> instrument (n = 66051.) "NumberIO.LE.read_float32: %f" n);
      test "NumberIO.BE.read_float32" (fun () ->
          Lwt_bytes.of_string "\x47\x81\x01\x80"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_float32
          >|= fun n -> instrument (n = 66051.) "NumberIO.BE.read_float32: %f" n);
      test "NumberIO.LE.read_float64" (fun () ->
          Lwt_bytes.of_string "\x70\x60\x50\x40\x30\x20\xf0\x42"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.LE.read_float64
          >|= Int64.bits_of_float
          >|= ( = ) 0x42F0203040506070L);
      test "NumberIO.BE.read_float64" (fun () ->
          Lwt_bytes.of_string "\x42\xf0\x20\x30\x40\x50\x60\x70"
          |> Lwt_io.(of_bytes ~mode:input)
          |> Lwt_io.BE.read_float64
          >|= Int64.bits_of_float
          >|= ( = ) 0x42F0203040506070L);
      test "NumberIO.LE.write_int" (fun () ->
          let buffer = Lwt_bytes.create 4 in
          Lwt_io.LE.write_int (Lwt_io.(of_bytes ~mode:output) buffer) 0x01020304
          >>= fun () ->
          Lwt.return (Lwt_bytes.to_string buffer = "\x04\x03\x02\x01"));
      test "NumberIO.BE.write_int" (fun () ->
          let buffer = Lwt_bytes.create 4 in
          Lwt_io.BE.write_int (Lwt_io.(of_bytes ~mode:output) buffer) 0x01020304
          >>= fun () ->
          Lwt.return (Lwt_bytes.to_string buffer = "\x01\x02\x03\x04"));
      test "NumberIO.LE.write_int16" (fun () ->
          let buffer = Lwt_bytes.create 2 in
          Lwt_io.LE.write_int16 (Lwt_io.(of_bytes ~mode:output) buffer) 0x0102
          >>= fun () -> Lwt.return (Lwt_bytes.to_string buffer = "\x02\x01"));
      test "NumberIO.BE.write_int16" (fun () ->
          let buffer = Lwt_bytes.create 2 in
          Lwt_io.BE.write_int16 (Lwt_io.(of_bytes ~mode:output) buffer) 0x0102
          >>= fun () -> Lwt.return (Lwt_bytes.to_string buffer = "\x01\x02"));
      test "NumberIO.LE.write_int32" (fun () ->
          let buffer = Lwt_bytes.create 4 in
          Lwt_io.LE.write_int32
            (Lwt_io.(of_bytes ~mode:output) buffer)
            0x01020304l
          >>= fun () ->
          Lwt.return (Lwt_bytes.to_string buffer = "\x04\x03\x02\x01"));
      test "NumberIO.BE.write_int32" (fun () ->
          let buffer = Lwt_bytes.create 4 in
          Lwt_io.BE.write_int32
            (Lwt_io.(of_bytes ~mode:output) buffer)
            0x01020304l
          >>= fun () ->
          Lwt.return (Lwt_bytes.to_string buffer = "\x01\x02\x03\x04"));
      test "NumberIO.LE.write_int64" (fun () ->
          let buffer = Lwt_bytes.create 8 in
          Lwt_io.LE.write_int64
            (Lwt_io.(of_bytes ~mode:output) buffer)
            0x0102030405060708L
          >>= fun () ->
          Lwt.return
            (Lwt_bytes.to_string buffer = "\x08\x07\x06\x05\x04\x03\x02\x01"));
      test "NumberIO.BE.write_int64" (fun () ->
          let buffer = Lwt_bytes.create 8 in
          Lwt_io.BE.write_int64
            (Lwt_io.(of_bytes ~mode:output) buffer)
            0x0102030405060708L
          >>= fun () ->
          Lwt.return
            (Lwt_bytes.to_string buffer = "\x01\x02\x03\x04\x05\x06\x07\x08"));
      test "NumberIO.LE.write_float32" (fun () ->
          let buffer = Lwt_bytes.create 4 in
          Lwt_io.LE.write_float32 (Lwt_io.(of_bytes ~mode:output) buffer) 66051.
          >|= fun () ->
          instrument
            (Lwt_bytes.to_string buffer = "\x80\x01\x81\x47")
      ...
      test "NumberIO.BE.write_float32" (fun () ->
          let buffer = Lwt_bytes.create 4 in
          Lwt_io.BE.write_float32 (Lwt_io.(of_bytes ~mode:output) buffer) 66051.
          >|= fun () ->
          instrument
            (Lwt_bytes.to_string buffer = "\x47\x81\x01\x80")
            "NumberIO.BE.write_float32: %02X %02X %02X %02X"
            (Char.code (Lwt_bytes.get buffer 0))
            (Char.code (Lwt_bytes.get buffer 1))
            (Char.code (Lwt_bytes.get buffer 2))
            (Char.code (Lwt_bytes.get buffer 3)));
      test "NumberIO.LE.write_float64" (fun () ->
          let buffer = Lwt_bytes.create 8 in
          Lwt_io.LE.write_float64
            (Lwt_io.(of_bytes ~mode:output) buffer)
            (Int64.float_of_bits 0x42F0203040506070L)
          >>= fun () ->
          Lwt.return
            (Lwt_bytes.to_string buffer = "\x70\x60\x50\x40\x30\x20\xf0\x42"));
      test "NumberIO.BE.write_float64" (fun () ->
          let buffer = Lwt_bytes.create 8 in
          Lwt_io.BE.write_float64
            (Lwt_io.(of_bytes ~mode:output) buffer)
            (Int64.float_of_bits 0x42F0203040506070L)
          >>= fun () ->
          Lwt.return
            (Lwt_bytes.to_string buffer = "\x42\xf0\x20\x30\x40\x50\x60\x70"));
      test "Write from Lwt_bytes" (fun () ->
          let bytes = Lwt_bytes.of_string "Hello World" in
          let out = Lwt_bytes.create 11 in
          Lwt_io.write_from_exactly_bigstring
            (Lwt_io.(of_bytes ~mode:output) out)
            bytes 0 11
          >>= fun () -> Lwt.return (Lwt_bytes.to_string out = "Hello World"));
      test "Read from Lwt_bytes" (fun () ->
          let bytes_in = Lwt_bytes.create 11 in
          let bytes = Lwt_bytes.of_string "Hello World" in
          Lwt_io.read_into_exactly_bigstring
            (Lwt_io.(of_bytes ~mode:input) bytes)
            bytes_in 0 11
          >>= fun () -> Lwt.return (Lwt_bytes.to_string bytes_in = "Hello World"));
    ]
