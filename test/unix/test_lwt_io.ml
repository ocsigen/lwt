    ...
    Lwt_io.establish_server_with_client_address
      ...
      (fun _client_address channels ->
        Lwt.finalize
          (fun () -> f channels)
          (fun () ->
            Lwt.awaken ~order:Nested notify_handler_finished ();
            Lwt.return_unit))
    ...
    (fun () ->
      ...
      let server =
        (Lwt_io.Versioned.establish_server_1 [@ocaml.warning "-3"])
          local (fun channels -> Lwt.awaken ~order:Nested run_handler channels)
      in

      Lwt_io.with_connection local (fun _ -> Lwt.return_unit) >>= fun () ->
      Lwt.awaken ~order:Nested client_finished ();
        ...
        (Lwt_io.Versioned.establish_server_1 [@ocaml.warning "-3"])
          local (fun (in_channel, out_channel) ->
            Lwt.async (fun () ->
              Lwt_io.close in_channel >>= fun () ->
              Lwt_io.close out_channel >|= fun () ->
              Lwt.awaken ~order:Nested server_finished ()))
      in
