...
let tcp_server_client_exchange server_logic client_logic =
  ...
  let server () =
    ...
    Lwt_unix.accept sock >>= fun (fd_client, _) ->
    server_logic fd_client >>= fun _n ->
    Lwt_unix.close fd_client >>= fun () -> Lwt_unix.close sock
  in
  let client () =
    server_is_ready >>= fun sockaddr ->
    let sock = Lwt_unix.socket Lwt_unix.PF_INET Lwt_unix.SOCK_STREAM 0 in
    Lwt_unix.connect sock sockaddr >>= fun () ->
    client_logic sock >>= fun _n -> Lwt_unix.close sock
  in
...
let udp_server_client_exchange server_logic client_logic =
  ...
  let server () =
    ...
    let server_address = Lwt_unix.getsockname sock in
    Lwt.wakeup_later notify_server_is_ready server_address;
    server_logic sock >>= fun (_n, _sockaddr) -> Lwt_unix.close sock
  in
  let client () =
    server_is_ready >>= fun sockaddr ->
    let sock = Lwt_unix.socket Lwt_unix.PF_INET Lwt_unix.SOCK_DGRAM 0 in
    client_logic sock sockaddr >>= fun _n -> Lwt_unix.close sock
  in
  Lwt.join [ client (); server () ]
...
let test_mincore buff_len offset n_states =
  ...
  Lwt_bytes.write fd buf_write 0 buff_len >>= fun _n ->
  ...
  suite "lwt_bytes"
    [
      ...
      test "of bytes" (fun () ->
          ...
          let check =
            Lwt_bytes.get buff 0 = Bytes.get bytes 0
      ...
      test "blit" (fun () ->
          ...
          Lwt.return check);
      test "blit source out of bounds: lower limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return true
          | () -> Lwt.return false);
      test "blit source out of bounds: upper limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return true
          | () -> Lwt.return false);
      test "blit destination out of bounds: lower limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return true
          | () -> Lwt.return false);
      test "blit destination out of bounds: upper limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return true
          | () -> Lwt.return false);
      test "blit length out of bounds: lower limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
      ...
      test "blit from bytes" (fun () ->
          ...
          let check = "abcabc" = Lwt_bytes.to_string buf2 in
          Lwt.return check);
      test "blit from bytes source out of bounds: lower limit" (fun () ->
          let bytes1 = Bytes.of_string "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | () -> Lwt.return_false);
      test "blit from bytes source out of bounds: upper limit" (fun () ->
          let bytes1 = Bytes.of_string "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          match Lwt_bytes.blit_from_bytes bytes1 1 buf2 3 3 with
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from bytes destination out of bounds: lower limit" (fun () ->
          let bytes1 = Bytes.of_string "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from bytes destination out of bounds: upper limit" (fun () ->
          let bytes1 = Bytes.of_string "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from bytes length out of bounds: lower limit" (fun () ->
          let bytes1 = Bytes.of_string "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
      ...
      test "blit from string" (fun () ->
          ...
          let check = "abcabc" = Lwt_bytes.to_string buf2 in
          Lwt.return check);
      test "blit from string source out of bounds: lower limit" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from string source out of bounds: upper limit" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          match Lwt_bytes.blit_from_string string1 1 buf2 3 3 with
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from string destination out of bounds: lower limit" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from string destination out of bounds: upper limit" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from string length out of bounds: lower limit" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit from string length out of bounds: upper limit" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
      ...
      test "blit to bytes" (fun () ->
          ...
          Lwt.return check);
      test "blit to bytes source out of bounds: lower limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let bytes2 = Bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit to bytes source out of bounds: upper limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let bytes2 = Bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit to bytes destination out of bounds: lower limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let bytes2 = Bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit to bytes destination out of bounds: upper limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let bytes2 = Bytes.of_string str2 in
          ...
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "blit to bytes length out of bounds: lower limit" (fun () ->
          let str1 = "abc" in
          let buf1 = Lwt_bytes.of_string str1 in
          let str2 = "abcdef" in
          let bytes2 = Bytes.of_string str2 in
      ...
      test "unsafe blit" (fun () ->
          ...
          let check = "abcabc" = Lwt_bytes.to_string buf2 in
          Lwt.return check);
      test "unsafe blit from bytes" (fun () ->
          let bytes1 = Bytes.of_string "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
          ...
          let check = "abcabc" = Lwt_bytes.to_string buf2 in
          Lwt.return check);
      test "unsafe blit from string" (fun () ->
          let string1 = "abc" in
          let str2 = "abcdef" in
          let buf2 = Lwt_bytes.of_string str2 in
      ...
      test "proxy offset out of bounds: lower limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.proxy buf (-1) 3 with
          | exception Invalid_argument _ -> Lwt.return_true
          | _ -> Lwt.return_false);
      test "proxy offset out of bounds: upper limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.proxy buf 4 3 with
          | exception Invalid_argument _ -> Lwt.return_true
          | _ -> Lwt.return_false);
      test "proxy length out of bounds: lower limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
      ...
      test "extract" (fun () ->
          ...
          Lwt.return check);
      test "extract offset out of bounds: lower limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.extract buf (-1) 3 with
          | exception Invalid_argument _ -> Lwt.return_true
          | _ -> Lwt.return_false);
      test "extract offset out of bounds: upper limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.extract buf 4 3 with
          | exception Invalid_argument _ -> Lwt.return_true
          | _ -> Lwt.return_false);
      test "extract length out of bounds: lower limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
      ...
      test "fill" (fun () ->
          ...
          Lwt.return check);
      test "fill offset out of bounds: lower limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.fill buf (-1) 3 'a' with
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "fill offset out of bounds: upper limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.fill buf 4 3 'a' with
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "fill length out of bounds lower limit" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
          match Lwt_bytes.fill buf 3 (-1) 'a' with
          | exception Invalid_argument _ -> Lwt.return_true
          | () -> Lwt.return_false);
      test "unsafe fill" (fun () ->
          let str = "abcdef" in
          let buf = Lwt_bytes.of_string str in
      ...
      test "bytes read" (fun () ->
          ...
          Lwt_unix.openfile test_file [ O_RDONLY ] 0 >>= fun fd ->
          let buf = Lwt_bytes.create 6 in
          Lwt_bytes.read fd buf 0 6 >>= fun _n ->
          let check = "abcdef" = Lwt_bytes.to_string buf in
          Lwt_unix.close fd >>= fun () -> Lwt.return check);
      test "read: buffer retention" ~sequential:true (fun () ->
          ...
          Lwt_unix.close read_fd >|= fun () -> !retained);
      test "bytes write" (fun () ->
          ...
          Lwt_bytes.write fd buf_write 0 3 >>= fun _n ->
          Lwt_unix.close fd >>= fun () ->
          Lwt_unix.openfile test_file [ O_RDONLY ] 0 >>= fun fd ->
          let buf_read = Lwt_bytes.create 3 in
          Lwt_bytes.read fd buf_read 0 3 >>= fun _n ->
          let check = buf_write = buf_read in
          Lwt_unix.close fd >>= fun () -> Lwt.return check);
      test "write: buffer retention" ~sequential:true (fun () ->
          let buffer = Lwt_bytes.create 3 in
      ...
      test "bytes recv"
        ...
        (fun () ->
          ...
          in
          let client_logic socket = Lwt_bytes.recv socket buf 0 6 [] in
          tcp_server_client_exchange server_logic client_logic >>= fun () ->
          let check = "abcdef" = Lwt_bytes.to_string buf in
          Lwt.return check);
      test "bytes send"
        ~only_if:(fun () -> not Sys.win32)
      ...
      test "bytes recvfrom"
        ...
        (fun () ->
          let buf = gen_buf 6 in
          let server_logic socket = Lwt_bytes.recvfrom socket buf 0 6 [] in
          let client_logic socket sockaddr =
            Lwt_unix.sendto socket
              (Bytes.of_string "abcdefghij")
              0 9 [] sockaddr
          in
          udp_server_client_exchange server_logic client_logic >>= fun () ->
          let check = "abcdef" = Lwt_bytes.to_string buf in
          Lwt.return check);
      test "bytes sendto"
        ~only_if:(fun () -> not Sys.win32)
      ...
      test "bytes send_msg"
        ...
        (fun () ->
          ...
          let server_logic socket =
            let io_vectors =
              [ Lwt_bytes.io_vector ~buffer ~offset ~length:6 ]
            in
            (Lwt_bytes.recv_msg [@ocaml.warning "-3"]) ~socket ~io_vectors
          in
          let client_logic socket sockaddr =
            ...
            let message = Lwt_bytes.of_string "abcdefghij" in
            let io_vectors =
              [ Lwt_bytes.io_vector ~buffer:message ~offset ~length:9 ]
            in
            (Lwt_bytes.send_msg [@ocaml.warning "-3"]) ~socket ~io_vectors
              ~fds:[]
          in
          udp_server_client_exchange server_logic client_logic >>= fun () ->
          let check = "abcdef" = Lwt_bytes.to_string buffer in
        ...
        (fun () ->
          Lwt.catch
            (fun () ->
              ...
              test_wait_mincore buff_len (buff_len + 1) >>= fun () ->
              Lwt.return false)
            (function
              | Invalid_argument _message -> Lwt.return true
              | exn -> Lwt.fail exn));
