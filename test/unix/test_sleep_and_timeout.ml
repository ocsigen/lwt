...
let cmp_elapsed_time test_name start_time expected_time =
  ...
  instrument result "Lwt_unix sleep and timeout: %s: %f %f %f %b" test_name
    elapsed_time expected_time diff
    (Lwt_sys.have `libev)

let suite =
  suite "Lwt_unix sleep and timeout"
    [
      test "sleep" (fun () ->
      ...
      test "timeout" (fun () ->
          ...
          let duration = 1.0 in
          Lwt.catch
            (fun () -> Lwt_unix.timeout duration >>= fun () -> Lwt.return_false)
            (function
              | Lwt_unix.Timeout ->
                  let check = cmp_elapsed_time "timeout" start_time duration in
                  Lwt.return check
              | exn -> Lwt.fail exn));
      test "with_timeout : no timeout" (fun () ->
          let duration = 1.0 in
          Lwt_unix.with_timeout duration Lwt.pause >>= fun () -> Lwt.return_true);
      test "with_timeout : timeout" (fun () ->
          let start_time = Unix.gettimeofday () in
          let duration = 1.0 in
          let f () = Lwt_unix.sleep 2.0 in
          Lwt.catch
            (fun () ->
              Lwt_unix.with_timeout duration f >>= fun () ->
              Printf.eprintf "\nno timeout\n";
              Lwt.return false)
            (function
              | Lwt_unix.Timeout ->
                  let check =
                    cmp_elapsed_time "with_timeout : timeout" start_time
                      duration
                  in
                  Lwt.return check
              | exn -> Lwt.fail exn));
      ...
      test "auto_pause" (fun () ->
          ...
          instrument check
            "Lwt_unix sleep and timeout: auto_pause: %b %b %b %b %b" check1
