  ...
  [
    test "libev: default when enabled in build bot" (fun () ->
        if not Lwt_config._HAVE_LIBEV then Lwt.return_true
          ...
          let in_appveyor =
            try
              ignore (Sys.getenv "APPVEYOR_REPO_COMMIT");
              true
            with Not_found -> false
          in
...
let timing_tests =
  [
    test "libev: timer delays are not too short" (fun () ->
        ...
        Lwt.catch
          ...
          (function
            | Lwt_unix.Timeout -> Lwt.return (Unix.gettimeofday ())
            | exn -> Lwt.fail exn)
        >>= fun stop -> Lwt.return (stop -. start >= 1.5));
  ]
...
let run_tests =
  [
    test "Lwt_main.run: nested call" ~sequential:true (fun () ->
        ...
        try
          ...
          Lwt.return false
        with Failure _ -> Lwt.return true);
  ]
