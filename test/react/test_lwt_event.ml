  ...
  suite "lwt_event"
    [
      ...
      test "of_stream" (fun () ->
          ...
          let l = ref [] in
          let event =
            React.E.map (fun x -> l := x :: !l) (Lwt_react.E.of_stream stream)
          in
          ignore event;
          push (Some 1);
          push (Some 2);
      ...
      test "limit" (fun () ->
          ...
          let event' =
            Lwt_react.E.limit (fun () -> Lwt_condition.wait cond) event
          in
          let l = ref [] in
          ...
          push 4;
          Lwt_condition.signal cond ();
          Lwt.pause () >>= fun () -> return (!l = [ 4; 3; 2; 1 ]));
      test "with_finaliser lifetime" (fun () ->
          let e, push = React.E.create () in
          let finalizer_ran = ref false in
          let e' =
            Lwt_react.E.with_finaliser (fun () -> finalizer_ran := true) e
          in
