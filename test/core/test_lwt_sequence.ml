...
let suite =
  suite "lwt_sequence"
    [
      ...
      test "take_l Empty" (fun () ->
          ...
          Lwt.catch
            (fun () ->
              ...
              Lwt.return_false)
            (function
              | Lwt_sequence.Empty -> Lwt.return_true | _ -> Lwt.return_false));
      test "take_l" (fun () ->
      ...
      test "iter_node_l Empty" (fun () ->
          test_iter_node Lwt_sequence.iter_node_l empty_array
            (Lwt_sequence.create ()));
      test "iter_node_l" (fun () ->
          test_iter_node Lwt_sequence.iter_node_l l_filled_array
            (filled_sequence ()));
      test "iter_node_r Empty" (fun () ->
          test_iter_node Lwt_sequence.iter_node_r empty_array
            (Lwt_sequence.create ()));
      test "iter_node_r" (fun () ->
          test_iter_node Lwt_sequence.iter_node_r r_filled_array
            (filled_sequence ()));
      test "iter_node_l with removal" (fun () ->
          test_iter_rem Lwt_sequence.iter_node_l l_filled_array
            (filled_sequence ()));
      test "iter_node_r with removal" (fun () ->
          test_iter_rem Lwt_sequence.iter_node_r r_filled_array
            (filled_sequence ()));
      test "fold_l" (fun () ->
          let acc =
            Lwt_sequence.fold_l (fun v e -> v * e) (filled_sequence ()) 1
          in
          Lwt.return (factorial_sequence = acc));
      test "fold_l Empty" (fun () ->
          let acc =
            Lwt_sequence.fold_l (fun v e -> v * e) (Lwt_sequence.create ()) 1
          in
          Lwt.return (acc = 1));
      test "fold_r" (fun () ->
          let acc =
            Lwt_sequence.fold_r (fun v e -> v * e) (filled_sequence ()) 1
          in
          Lwt.return (factorial_sequence = acc));
      test "fold_r Empty" (fun () ->
          let acc =
            Lwt_sequence.fold_r (fun v e -> v * e) (Lwt_sequence.create ()) 1
          in
          Lwt.return (acc = 1));
      test "find_node_opt_l Empty" (fun () ->
          let s = Lwt_sequence.create () in
      ...
      test "find_node_opt_r Empty" (fun () ->
          let s = Lwt_sequence.create () in
          match Lwt_sequence.find_node_opt_r (fun v -> v = 1) s with
          | None -> Lwt.return_true
          | _ -> Lwt.return_false);
      test "find_node_opt_r not found " (fun () ->
          let s = transfer_sequence () in
          match Lwt_sequence.find_node_opt_r (fun v -> v = 1) s with
          | None -> Lwt.return_true
          | _ -> Lwt.return_false);
      test "find_node_opt_r" (fun () ->
          let s = filled_sequence () in
          match Lwt_sequence.find_node_opt_r (fun v -> v = 1) s with
          | None -> Lwt.return_false
          | Some n ->
      ...
      test "set" (fun () ->
          ...
          | None -> Lwt.return_false
          | Some n ->
              let _ = Lwt_sequence.set n 10 in
              let data = [| 1; 2; 3; 10; 5; 6 |] in
              test_iter Lwt_sequence.iter_l data s);
      test "fold_r with multiple removal" (fun () ->
          let s = filled_sequence () in
      ...
      test "fold_l multiple removal" (fun () ->
          let s = filled_sequence () in
          let n_four = Lwt_sequence.find_node_r (fun v' -> v' = 4) s in
      ...
      test "find_node_r with multiple removal" (fun () ->
          let s = filled_sequence () in
          let n_three = Lwt_sequence.find_node_r (fun v' -> v' = 3) s in
          let n_two = Lwt_sequence.find_node_r (fun v' -> v' = 2) s in
          Lwt.catch
            (fun () ->
              ...
              Lwt.return (v = 1))
            (function _ -> Lwt.return_false));
      test "find_node_l with multiple removal" (fun () ->
          let s = filled_sequence () in
          let n_three = Lwt_sequence.find_node_r (fun v' -> v' = 3) s in
          let n_four = Lwt_sequence.find_node_r (fun v' -> v' = 4) s in
          Lwt.catch
