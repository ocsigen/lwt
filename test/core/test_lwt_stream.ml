...
let expect_exit f =
  Lwt.catch
    (fun () -> f () >>= fun _ -> Lwt.return_false)
    (function Exit -> Lwt.return_true | e -> Lwt.fail e)
  ...
  suite "lwt_stream"
    [
      test "from" (fun () ->
          ...
          let stream =
            Lwt_stream.from (fun () ->
                Lwt_mvar.take mvar >>= fun x -> return (Some x))
          in
          ...
          t1 >>= fun x1 ->
          t2 >>= fun x2 ->
          t3 >>= fun x3 -> return ([ x1; x2; x3 ] = [ 1; 1; 1 ]));
      test "return" (fun () ->
      ...
      test "return_lwt_with_pause" (fun () ->
          let lwt = Lwt.pause () >>= fun () -> Lwt.return 123 in
          let stream = Lwt_stream.return_lwt lwt in
          Lwt_stream.next stream >>= fun x ->
          return (x = 123 && Lwt_stream.is_closed stream));
      test "return_lwt_with_fail" (fun () ->
          let lwt = Lwt.pause () >>= fun () -> raise (Failure "not today no") in
          let stream = Lwt_stream.return_lwt lwt in
          Lwt.catch
            (fun () -> Lwt_stream.next stream >>= fun _ -> Lwt.return_false)
            (function Lwt_stream.Empty -> Lwt.return_true | exc -> raise exc));
      test "of_seq" (fun () ->
          let x = ref false in
          let nil () =
            x := not !x;
          ...
          return
            ([ closed_before; closed_after ] = [ false; true ]
      ...
      test "of_lwt_seq" (fun () ->
          ...
          let nil () =
            ...
            Lwt_seq.Nil
          in
          let seq () = Lwt.pause () >|= fun () -> Lwt_seq.Cons (1, nil) in
      ...
      test "clone" (fun () ->
          ...
          return
            ([ x1_1; x1_2; x1_3 ] = [ 1; 2; 3 ]
            && [ x2_1; x2_2; x2_3 ] = [ 1; 2; 3 ]));
      test "clone 2" (fun () ->
      ...
      test "create_bounded" (fun () ->
          ...
          let acc =
            acc && state (Lwt_stream.to_list stream) = Return [ 3; 4; 7 ]
          in
          return acc);
      test "create_bounded close" (fun () ->
      ...
      test "get_available_up_to" (fun () ->
          ...
          Lwt_stream.get stream >>= fun x -> return (l = [ 1; 2 ] && x = Some 3));
      test "filter" (fun () ->
          let stream, push = Lwt_stream.create () in
          push (Some 1);
          push (Some 2);
      ...
      test "filter_map" (fun () ->
          ...
          push (Some 4);
          let filtered =
            Lwt_stream.filter_map (function 3 -> Some "3" | _ -> None) stream
          in
          Lwt_stream.get filtered >>= fun x ->
          let l = Lwt_stream.get_available filtered in
          return (x = Some "3" && l = []));
      test "last_new" (fun () ->
          ...
          push (Some 2);
          push (Some 3);
          Lwt_stream.last_new stream >>= fun x -> return (x = 3));
      test "cancel push stream 1" (fun () ->
          let stream, _ = Lwt_stream.create () in
          let t = Lwt_stream.next stream in
          cancel t;
          return (state t = Fail Canceled));
      test "cancel push stream 2" (fun () ->
          let stream, push = Lwt_stream.create () in
          let t = Lwt_stream.next stream in
          cancel t;
          push (Some 1);
      ...
      test "on_termination" (fun () ->
          ...
          in
          let b = ref false in
          (Lwt_stream.on_termination [@ocaml.warning "-3"]) st (fun () ->
              b := true);
          ignore (Lwt_stream.peek st);
          let b1 = !b = false in
          ignore (Lwt_stream.junk st);
          ...
          Lwt.return (b1 && b2 && b3));
      test "on_termination when closed" (fun () ->
      ...
      test "exception passing: resume, not closed, from" (fun () ->
          ...
          Lwt.return
            ...
            && got_none
            && closed_at_end));
      test "exception passing: resume, not closed, from_direct" (fun () ->
