  ...
  suite "lwt_seq"
    [
      ...
      test "filter_map" (fun () ->
          ...
          let+ l' = Lwt_seq.to_list v in
          l' = [ 4; 8 ]);
      test "filter_map_s" (fun () ->
          let v =
            Lwt_seq.filter_map_s
              (fun x -> Lwt.return (if x mod 2 = 0 then Some (x * 2) else None))
              a
          in
          let+ l' = Lwt_seq.to_list v in
          l' = [ 4; 8 ]);
      ...
      test "unfold_lwt" (fun () ->
          ...
          let+ b = Lwt_seq.to_list (range 1 0) in
          [ 1; 2; 3 ] = a && [] = b);
      test "fold-into-exception-from-of-seq" (fun () ->
          let fail () = failwith "XXX" in
          let seq () = Seq.Cons (1, fun () -> Seq.Cons (2, fail)) in
          let a = Lwt_seq.of_seq seq in
          let+ n =
            Lwt.catch
              (fun () -> Lwt_seq.fold_left ( + ) 0 a)
              (function
                | Failure x when x = "XXX" -> Lwt.return (-1) | exc -> raise exc)
          in
          n = -1);
      test "fold-into-immediate-exception-from-of-seq" (fun () ->
          let fail () = failwith "XXX" in
          let seq = fail in
          let a = Lwt_seq.of_seq seq in
          let+ n =
          ...
          in
          n = -1);
      test "fold-into-exception-from-of-seq-lwt" (fun () ->
          let fail () = failwith "XXX" in
          let seq : int Lwt.t Seq.t =
           fun () ->
            Seq.Cons (Lwt.return 1, fun () -> Seq.Cons (Lwt.return 2, fail))
          in
          ...
          in
          n = -1);
      test "fold-into-immediate-exception-from-of-seq-lwt" (fun () ->
          let fail () = failwith "XXX" in
...
let with_flc test =
  Lwt_list.for_all_s
    (fun f ->
      Lwt_list.for_all_s
        (fun l -> Lwt_list.for_all_s (fun c -> test f l c) cs)
        ls)
    fs
...
let suite_fuzzing =
  suite "lwt_seq(pseudo-fuzzing)"
    [
          ...
          with_flc (fun f l c ->
              ...
              let sf s =
                ...
                fun () ->
                  ...
                  Lwt.return (Lwt_seq.Cons (!r, Lwt_seq.empty))
              in
              commutes lf sf l));
      test "iter_s" (fun () ->
          with_flc (fun f l c ->
      ...
      test "iter_n" (fun () ->
          l
          |> Lwt_list.for_all_s @@ fun max_concurrency ->
             with_flc (fun f l c ->
                 let lf l =
                   let r = ref c in
                   List.iter (fun x -> r := f !r x) l;
