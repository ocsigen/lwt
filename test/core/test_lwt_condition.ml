...
let suite =
  suite "lwt_condition"
    [
      ...
      test "mutex unlocked during wait" (fun () ->
          let c = Lwt_condition.create () in
          let m = Lwt_mutex.create () in
          let _ = Lwt_mutex.lock m in
          let w = Lwt_condition.wait ~mutex:m c in
          Lwt.return (Lwt.state w = Lwt.Sleep && not (Lwt_mutex.is_locked m)));
      test "mutex relocked after wait" (fun () ->
          let c = Lwt_condition.create () in
          let m = Lwt_mutex.create () in
          let _ = Lwt_mutex.lock m in
          let w = Lwt_condition.wait ~mutex:m c in
          let () = Lwt_condition.signal c 1 in
          Lwt.bind w (fun v -> Lwt.return (v = 1 && Lwt_mutex.is_locked m)));
      test "signal is not sticky" (fun () ->
          let c = Lwt_condition.create () in
          let () = Lwt_condition.signal c 1 in
          let w = Lwt_condition.wait c in
      ...
      test "broadcast" (fun () ->
          ...
          Lwt.bind w1 (fun v1 ->
              Lwt.bind w2 (fun v2 -> Lwt.return (v1 = 1 && v2 = 1))));
      test "broadcast exception" (fun () ->
          let c = Lwt_condition.create () in
          let w1 = Lwt_condition.wait c in
          let w2 = Lwt_condition.wait c in
