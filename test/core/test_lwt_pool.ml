...
let suite =
  suite "lwt_pool"
    [
      ...
      test "validation exceptions are propagated to users" (fun () ->
          ...
          Lwt.bind u1 (fun v1 ->
              Lwt.try_bind
                ...
                (fun exn2 -> Lwt.return (v1 = "done" && exn2 = Dummy_error))));
      test "multiple creation" (fun () ->
          let gen () =
            let n = ref 0 in
            Lwt.return n
          in
          ...
          let u2 = Lwt_pool.use p (fun n -> Lwt.return !n) in
          Lwt.return (Lwt.state u2 = Lwt.Return 0));
      test "users of an empty pool will wait" (fun () ->
          let gen () = Lwt.return 0 in
          let p = Lwt_pool.create 1 gen in
          let _ = Lwt_pool.use p (fun _ -> Lwt.pause ()) in
          let u2 = Lwt_pool.use p Lwt.return in
          Lwt.return (Lwt.state u2 = Lwt.Sleep));
      test "on check, good elements are retained" (fun () ->
          let gen () =
            let n = ref 1 in
            Lwt.return n
          in
      ...
      test "on check, bad elements are disposed of and replaced" (fun () ->
          let gen () =
            let n = ref 1 in
            Lwt.return n
          in
          let check n f = f (!n > 0) in
      ...
      test "clear disposes of all elements" (fun () ->
          let gen () =
            let n = ref 1 in
            Lwt.return n
          in
      ...
      test "waiter are notified on replacement" (fun () ->
          let c = Lwt_condition.create () in
          let gen () =
            let l = ref 0 in
            Lwt.return l
      ...
      test "waiter are notified on replacement exception" (fun () ->
          let c = Lwt_condition.create () in
          let k = ref true in
          let gen () =
            if !k then
      ...
      test "check and validate can be used together" (fun () ->
          let gen () =
            let l = ref 0 in
            Lwt.return l
          in
          ...
          Lwt.bind u3 (fun v -> Lwt.return (v = 2)));
      test "verify default check behavior" (fun () ->
          let gen () =
            let l = ref 0 in
            Lwt.return l
          in
          ...
          let _ =
            Lwt_pool.use p (fun l ->
                Lwt.bind (Lwt_condition.wait cond) (fun _ ->
                    l := 1;
                    Lwt.fail Dummy_error))
          in
          let u2 = Lwt_pool.use p (fun l -> Lwt.return !l) in
          let () = Lwt_condition.signal cond "done" in
          Lwt.bind u2 (fun v -> Lwt.return (v = 1)));
    ]
