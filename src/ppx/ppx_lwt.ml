...
let with_loc f { txt; loc } = f ~loc txt

(** Test if a case is a catchall. *)
let is_catchall case =
  let rec is_catchall_pat p =
...
let add_wildcard_case cases =
  let has_wildcard = List.exists is_catchall cases in
...
let gen_bindings l =
  let aux i binding =
    { binding with pvb_pat = pvar ~loc:binding.pvb_expr.pexp_loc (gen_name i) }
  in
  List.mapi aux l
...
let lwt_expression mapper exp attributes ext_loc =
  ...
  | Pexp_let (Nonrecursive, vbl, e) ->
      let new_exp =
        pexp_let ~loc:!default_loc Nonrecursive (gen_bindings vbl)
      ...
      let new_exp =
        ...
        | _ ->
            ...
            [%expr
              Lwt.try_bind
                (fun () -> [%e e])
                [%e pexp_function ~loc cases] [%e pexp_function ~loc exns]]
      in
      Some (mapper#expression { new_exp with pexp_attributes })
  (* [assert%lwt $e$] ≡
  ...
  *)
  | Pexp_for (({ ppat_desc = Ppat_var p_var; _ } as p), start, bound, dir, body)
    ->
      ...
      let exp_bound =
        let loc = bound.pexp_loc in
        [%expr __ppx_lwt_bound]
      in
      let pat_bound =
        let loc = bound.pexp_loc in
        [%pat? __ppx_lwt_bound]
        ...
        [%expr
          ...
          let rec __ppx_lwt_loop [%p p] =
            if [%e comp] [%e p'] [%e exp_bound] then Lwt.return_unit
              ...
              Lwt.bind [%e body] (fun () -> __ppx_lwt_loop ([%e op] [%e p'] 1))
          in
  ...
  object (self)
    ...
    method! structure structure =
      if !warned then super#structure structure
        ...
        structure
        |> warn_if
             !used_no_strict_sequence_option
    ...
    method! expression expr =
      ...
      | {
       ...
       _;
          ...
          match lwt_expression self exp expr.pexp_attributes ext_loc with
          | Some expr' -> expr'
          | None -> expr)
      (* [($e$)[%finally $f$]] ≡
      ...
      | [%expr [%finally [%e? _]]] | [%expr [%lwt.finally [%e? _]]] ->
...
let args =
  [
    ("-no-sequence", Arg.Unit no_sequence_option, " has no effect (deprecated)");
    ( "-no-strict-sequence",
...
let () =
  ...
  Driver.register_transformation "ppx_lwt" ~impl:mapper#structure
