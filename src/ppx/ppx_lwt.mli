...
(** {2 Ppx extensions}
    ...
    This Ppx extension adds various syntactic shortcut for lwt programming. It
    needs OCaml >= 4.02 and {{:https://github.com/alainfrisch/ppx_tools}
    ppx_tools}.
    ...
    {[
      let%lwt x = do_something1 () and y = do_something2 in
      code
    ]}

    will run [do_something1 ()] and [do_something2 ()], then bind their results
    to [x] and [y]. It is the same as:

    {[
      let t1 = do_something1 and t2 = do_something2 in
    ...
    Note that the [exn -> Lwt.fail exn] branch is automatically added when
    needed.
