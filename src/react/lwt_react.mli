...
(** This module is an overlay for the [React] module. You can open it instead of
    the [React] module in order to get all of [React]'s functions plus Lwt ones.
...
module E : sig
  ...
  (** [with_finaliser f e] returns an event [e'] which behave as [e], except
      that [f] is called when [e'] is garbage collected. *)
  ...
  (** [limit f e] limits the rate of [e] with [f].
      ...
      For example, to limit the rate of an event to 1 per second you can use:
      [limit (fun () -> Lwt_unix.sleep 1.0) event]. *)

  val from : (unit -> 'a Lwt.t) -> 'a event
  (** [from f] creates an event which occurs each time [f ()] returns a value.
      If [f] raises an exception, the event is just stopped. *)
  ...
  (** [of_stream stream] creates an event which occurs each time a value is
  ...
  (** [delay promise] is an event which does not occur until [promise] resolves.
      Then it behaves as the event returned by [promise]. *)
  ...
  (** The following functions behave as their [React] counterpart, except that
      they take functions that may yield.

      As usual the [_s] suffix is used when calls are serialized, and the [_p]
...
module S : sig
  ...
  (** [bind ?eq s f] is initially [f x] where [x] is the current value of [s].
      Each time [s] changes to a new value [y], [bind
        signal f] is set
      to [f y], until the next change of [signal]. *)
  ...
  val bind_s :
    ...
    ('a -> 'b signal Lwt.t) ->
    'b signal Lwt.t
  (** Same as {!bind} except that [f] returns a promise. Calls to [f] are
      serialized. *)

  (** {2 Lwt-specific utilities} *)

  val with_finaliser : (unit -> unit) -> 'a signal -> 'a signal
  (** [with_finaliser f s] returns a signal [s'] which behaves as [s], except
      that [f] is called when [s'] is garbage collected. *)

  val limit :
    ?eq:('a -> 'a -> bool) -> (unit -> unit Lwt.t) -> 'a signal -> 'a signal
  (** [limit f s] limits the rate of [s] update with [f].

      For example, to limit it to 1 per second, you can use:
      [limit
