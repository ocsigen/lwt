...
(** A stream holding values of type ['a].
    ...
    Naming convention: in this module, all functions applying a function to each
    element of a stream are suffixed by:
...
(** [from f] creates a stream from the given input function. [f] is called each
    time more input is needed, and the stream ends when [f] returns [None].
    ...
    read from the stream will cause another call to [f], which may succeed with
    a value. *)

val from_direct : (unit -> 'a option) -> 'a t
(** [from_direct f] does the same as {!from} but with a function that does not
    return a thread. It is preferred that this function be used rather than
    wrapping [f] into a function which returns a thread.

    The behavior when [f] raises an exception is the same as for {!from}, except
    that [f] does not produce a thread. *)
...
(** [create ()] returns a new stream and a push function.
    ...
    {{:https://ocaml.org/api/Stdlib.html#TYPEresult} [result]} stream. There is
...
exception Full
(** Exception raised by the push function of a bounded push-stream when the
    stream queue is full and a thread is already waiting to push an element. *)
...
class type ['a] bounded_push =
  object
    ...
    (** Change the size of the stream queue. Note that the new size can smaller
        than the current stream queue size.

        It raises [Invalid_argument] if [size < 0]. *)

    method push : 'a -> unit Lwt.t
    (** Pushes a new element to the stream. If the stream is full then it will
        block until one element is consumed. If another thread is already
        blocked on {!push}, it raises {!Full}. *)

    method close : unit
    (** Closes the stream. Any thread currently blocked on {!push} fails with
        {!Closed}. *)
...
(** [create_bounded size] returns a new stream and a bounded push source. The
    stream can hold a maximum of [size] elements. When this limit is reached,
    pushing a new element will block until one is consumed.

    Note that you cannot clone or parse (with {!parse}) a bounded stream. These
    functions will raise [Invalid_argument] if you try to do so.
...
val clone : 'a t -> 'a t
(** [clone st] clone the given stream. Operations on each stream will not affect
    the other.
...
(** [peek st] returns the first element of the stream, if any, without removing
    it. *)
...
(** [next st] removes and returns the next element of the stream or fails with
    {!Empty}, if the stream is empty. *)
...
(** [junk_while f st] removes all elements at the beginning of the streams which
    satisfy [f]. *)
...
(** [junk_old st] removes all elements that are ready to be read without
    ...
    For example, the [read_password] function of [Lwt_read_line] uses it to
    flush keys previously typed by the user. *)
...
(** [is_closed st] returns whether the given stream has been closed. A closed
    ...
    [is_closed s = true], then all subsequent reads until the end of the stream
...
(** [on_termination st f] executes [f] when the end of the stream [st] is
    reached. Note that the stream may still contain elements if {!peek} or
    similar was used.
...
(** [choose l] creates an stream from a list of streams. The resulting stream
    will return elements returned by any stream of [l] in an unspecified order. *)
...
(** [map_list f st] applies [f] on each element of [st] and flattens the lists
    returned *)
...
(** [iter_n ?max_concurrency f s] iterates over all elements of the stream [s].
    ...
    Iteration is {b not} guaranteed to be in order as this function will attempt
    to always process [max_concurrency] elements from [s] at once.
...
(** [combine s1 s2] combines two streams. The stream will end when either stream
    ends. *)

val append : 'a t -> 'a t -> 'a t
(** [append s1 s2] returns a stream which returns all elements of [s1], then all
    elements of [s2] *)
...
(** [hexdump byte_stream] returns a stream which is the same as the output of
    [hexdump -C].
...
(** [map_exn s] returns a stream that captures all exceptions raised by the
    ...
    Note that for push-streams (as returned by {!create}) all elements of the
    mapped streams are values.
