...
(** Apply the function to every element; if [f x = None] then [x] is dropped; if
    [f x = Some y] then [y] is returned. This transformation is lazy, it only
...
(** Traverse the sequence from left to right, combining each element with the
    ...
    will not terminate (i.e., the promise will not resolve) on infinite
    sequences. *)
...
(** Build a sequence from a step function and an initial value. [unfold f u]
    ...
    [fun () -> Lwt.return (Cons (x, unfold f y))] if the promise [f u] resolves
    to [Some (x, y)]. *)
...
(** Convert a sequence to a list, preserving order. The traversal happens
    immediately and will not terminate (i.e., the promise will not resolve) on
    infinite sequences. *)
...
(** Convert from ['a Lwt.t Stdlib.Seq.t] to ['a Lwt_seq.t]. This transformation
    is lazy, it only applies when the result is traversed. *)
