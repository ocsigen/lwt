...
let tail_recursive_map f l = List.rev (List.rev_map f l)
...
let rec iter_s f l =
  match l with
  | [] -> Lwt.return_unit
  | x :: l -> Lwt.apply f x >>= fun () -> iter_s f l
...
let rec iteri_s i f l =
  match l with
  | [] -> Lwt.return_unit
  | x :: l -> Lwt.apply (f i) x >>= fun () -> iteri_s (i + 1) f l
...
let map_s f l =
  let rec inner acc = function
    ...
    | hd :: tl ->
        Lwt.apply f hd >>= fun r -> (inner [@ocaml.tailcall]) (r :: acc) tl
  in
  inner [] l

let rec _collect_rev acc = function
  | [] -> Lwt.return acc
  | t :: ts -> t >>= fun i -> (_collect_rev [@ocaml.tailcall]) (i :: acc) ts
...
let rec rev_map_append_s acc f l =
  match l with
  | [] -> Lwt.return acc
  | x :: l -> Lwt.apply f x >>= fun x -> rev_map_append_s (x :: acc) f l
...
let rev_map_p f l = rev_map_append_p Lwt.return_nil f l

let rec fold_left_s f acc l =
  match l with
  | [] -> Lwt.return acc
  | x :: l ->
      Lwt.apply (f acc) x >>= fun acc -> (fold_left_s [@ocaml.tailcall]) f acc l

let fold_right_s f l acc =
  let rec inner f a = function
    | [] -> Lwt.return a
    | hd :: tl ->
        Lwt.apply (f hd) a >>= fun a' -> (inner [@ocaml.tailcall]) f a' tl
  in
...
let rec for_all_s f l =
  ...
  | x :: l -> (
      ...
      | true -> (for_all_s [@ocaml.tailcall]) f l
...
let rec exists_s f l =
  ...
  | x :: l -> (
      ...
      | false -> (exists_s [@ocaml.tailcall]) f l)
...
let rec find_s f l =
  ...
  | x :: l -> (
      Lwt.apply f x >>= function
      | true -> Lwt.return x
      | false -> (find_s [@ocaml.tailcall]) f l)

let _optionalize f x =
  f x >>= fun b -> if b then Lwt.return (Some x) else Lwt.return None

let filter_s f l = filter_map_s (_optionalize f) l
let filter_p f l = filter_map_p (_optionalize f) l

let partition_s f l =
  let rec inner acc1 acc2 = function
    | [] -> Lwt.return (List.rev acc1, List.rev acc2)
    | hd :: tl ->
        Lwt.apply f hd >>= fun b ->
        if b then inner (hd :: acc1) acc2 tl else inner acc1 (hd :: acc2) tl
  in
  inner [] [] l
...
let partition_p f l =
  ...
  let group2 =
    tail_recursive_map snd @@ List.filter (fun x -> not @@ fst x) tl
  in
