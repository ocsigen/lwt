...
let iter_n ?(max_concurrency = 1) f seq =
  (if max_concurrency <= 0 then
   let message =
     Printf.sprintf "Lwt_seq.iter_n: max_concurrency must be > 0, %d given"
       max_concurrency
   in
   invalid_arg message);
  let rec loop running available seq =
    ...
    seq () >>= function
    | Nil -> Lwt.join running
    | Cons (elt, seq) -> loop (f elt :: running) (pred available) seq
  in
...
let rec of_list = function [] -> empty | h :: t -> cons h (of_list t)
...
let rec of_seq seq () =
  match seq () with
  | Seq.Nil -> return_nil
  | Seq.Cons (x, next) -> Lwt.return (Cons (x, of_seq next))
  | exception exn -> Lwt.fail exn

let rec of_seq_lwt (seq : 'a Lwt.t Seq.t) : 'a t =
 fun () ->
  match seq () with
  | Seq.Nil -> return_nil
  | Seq.Cons (x, next) ->
...
let of_seq_lwt (seq : 'a Lwt.t Seq.t) : 'a t =
 fun () ->
  match seq () with
  | Seq.Nil -> return_nil
  | Seq.Cons (x, next) ->
