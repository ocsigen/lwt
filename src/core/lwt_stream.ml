...
class ['a] bounded_push_impl (info : 'a push_bounded) wakener_cell last close =
  object
    ...
    method resize size =
      ...
      if info.pushb_count < info.pushb_size && info.pushb_pending <> None then (
        info.pushb_count <- info.pushb_count + 1;
        notify_pusher info last)

    method push x =
      if closed then Lwt.fail Closed
      else if info.pushb_pending <> None then Lwt.fail Full
      else if info.pushb_count >= info.pushb_size then (
...
let feed s =
  ...
  | From from ->
      ...
      if Lwt.is_sleeping from.from_thread then Lwt.protected from.from_thread
      else
...
let rec peek_rec s node =
  if node == !(s.last) then feed s >>= fun () -> peek_rec s node
...
let rec npeek_rec node acc n s =
  ...
  else if node == !(s.last) then feed s >>= fun () -> npeek_rec node acc n s
  else
...
let rec get_rec s node =
  if node == !(s.last) then feed s >>= fun () -> get_rec s node
  else (
...
let rec nget_rec node acc n s =
  if n <= 0 then Lwt.return (List.rev acc)
  else if node == !(s.last) then feed s >>= fun () -> nget_rec node acc n s
  else
...
let rec get_while_rec node acc f s =
  if node == !(s.last) then feed s >>= fun () -> get_while_rec node acc f s
  else
    match node.data with
    | Some x ->
        let test = f x in
...
let rec get_while_s_rec node acc f s =
  if node == !(s.last) then feed s >>= fun () -> get_while_s_rec node acc f s
  else
    match node.data with
    | Some x -> (
        f x >>= function
...
let rec next_rec s node =
  if node == !(s.last) then feed s >>= fun () -> next_rec s node
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec to_list_rec node acc s =
  if node == !(s.last) then feed s >>= fun () -> to_list_rec node acc s
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec to_string_rec node buf s =
  if node == !(s.last) then feed s >>= fun () -> to_string_rec node buf s
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec njunk_rec node n s =
  ...
  else if node == !(s.last) then feed s >>= fun () -> njunk_rec node n s
  else
    match node.data with
    | Some _ ->
        consume s node;
...
let rec junk_while_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> junk_while_rec node f s
  else
    match node.data with
    | Some x ->
        let test = f x in
...
let rec junk_while_s_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> junk_while_s_rec node f s
  else
    match node.data with
    | Some x -> (
        f x >>= function
...
let rec junk_old_rec node s =
  if node == !(s.last) then
    let thread = feed s in
    match Lwt.state thread with
    | Lwt.Return _ -> junk_old_rec node s
    | Lwt.Fail exn -> Lwt.fail exn
    | Lwt.Sleep -> Lwt.return_unit
  else
    match node.data with
    | Some _ ->
        consume s node;
...
let rec is_empty s =
  if s.node == !(s.last) then feed s >>= fun () -> is_empty s
...
let rec fold_rec node f s acc =
  if node == !(s.last) then feed s >>= fun () -> fold_rec node f s acc
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec fold_s_rec node f s acc =
  if node == !(s.last) then feed s >>= fun () -> fold_s_rec node f s acc
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec iter_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> iter_rec node f s
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec iter_s_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> iter_s_rec node f s
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec iter_p_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> iter_p_rec node f s
  else
    match node.data with
    | Some x ->
        consume s node;
...
let iter_n ?(max_concurrency = 1) f stream =
  ...
  let rec loop running available =
    (if available > 0 then Lwt.return (running, available)
    else
...
let rec find_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> find_rec node f s
  else
    match node.data with
    | Some x as opt ->
        consume s node;
...
let rec find_s_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> find_s_rec node f s
  else
    match node.data with
    | Some x as opt -> (
        consume s node;
...
let rec find_map_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> find_map_rec node f s
  else
    match node.data with
    | Some x ->
        consume s node;
...
let rec find_map_s_rec node f s =
  if node == !(s.last) then feed s >>= fun () -> find_map_s_rec node f s
  else
    match node.data with
    | Some x -> (
        consume s node;
...
let append s1 s2 =
  ...
  let rec next () =
    ...
    | Some _ -> t
    | None ->
        if !current_s == s2 then Lwt.return_none
        else (
          current_s := s2;
          next ())
  in
...
let hexdump stream =
  ...
  from (fun _ ->
      ...
      | l ->
          ...
          and blanks pos =
            if pos < 16 then (
              if pos = 8 then Buffer.add_string buf "    "
