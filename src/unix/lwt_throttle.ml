...
module Make (H : Hashtbl.HashedType) : S with type key = H.t = struct
  ...
  type key = H.t
  type elt = { mutable consumed : int; queue : bool Lwt.u Queue.t }
  ...
  let create ~rate ~max ~n =
    if rate < 1 || max < 1 || n < 0 then invalid_arg "Lwt_throttle.S.create"
    else { rate; max; waiting = 0; table = MH.create n; cleaning = None }
  ...
  let really_wait t elt =
    let w, u = Lwt.task () in
    if t.max > t.waiting then (
      Queue.add u elt.queue;
  ...
  let wait t key =
    let res =
      try
        ...
        if elt.consumed >= t.rate then really_wait t elt
        else (
          elt.consumed <- succ elt.consumed;
          Lwt.return_true)
      with Not_found ->
        let elt = { consumed = 1; queue = Queue.create () } in
        MH.add t.table key elt;
        Lwt.return_true
    in
    (match t.cleaning with None -> launch_cleaning t | Some _ -> ());
    res
end
