...
let fill bytes ofs len ch =
  ...
  else unsafe_fill bytes ofs len ch
...
external unsafe_blit_from_bytes : Bytes.t -> int -> t -> int -> int -> unit
  = "lwt_unix_blit_from_bytes" "noalloc"

external unsafe_blit_from_string : string -> int -> t -> int -> int -> unit
  = "lwt_unix_blit_from_string" "noalloc"

external unsafe_blit_to_bytes : t -> int -> Bytes.t -> int -> int -> unit
  = "lwt_unix_blit_to_bytes" "noalloc"

external unsafe_blit : t -> int -> t -> int -> int -> unit
  = "lwt_unix_blit" "noalloc"
...
let blit_from_string src_buf src_ofs dst_buf dst_ofs len =
  if
    len < 0
    || src_ofs < 0
    || src_ofs > String.length src_buf - len
    || dst_ofs < 0
    || dst_ofs > length dst_buf - len
  then invalid_arg "Lwt_bytes.blit_from_string"
  else unsafe_blit_from_string src_buf src_ofs dst_buf dst_ofs len

let blit_from_bytes src_buf src_ofs dst_buf dst_ofs len =
  if
    len < 0
    || src_ofs < 0
    || src_ofs > Bytes.length src_buf - len
    || dst_ofs < 0
    || dst_ofs > length dst_buf - len
  then invalid_arg "Lwt_bytes.blit_from_bytes"
  else unsafe_blit_from_bytes src_buf src_ofs dst_buf dst_ofs len

let blit_to_bytes src_buf src_ofs dst_buf dst_ofs len =
  if
    len < 0
    || src_ofs < 0
    || src_ofs > length src_buf - len
    || dst_ofs < 0
    || dst_ofs > Bytes.length dst_buf - len
  then invalid_arg "Lwt_bytes.blit_to_bytes"
  else unsafe_blit_to_bytes src_buf src_ofs dst_buf dst_ofs len
...
let blit src_buf src_ofs dst_buf dst_ofs len =
  if
    ...
    || dst_ofs < 0
    || dst_ofs > length dst_buf - len
  then invalid_arg "Lwt_bytes.blit"
  else unsafe_blit src_buf src_ofs dst_buf dst_ofs len
...
let read = (Lwt_unix.read_bigarray "Lwt_bytes.read" [@ocaml.warning "-3"])
let write = (Lwt_unix.write_bigarray "Lwt_bytes.write" [@ocaml.warning "-3"])

external stub_recv :
  Unix.file_descr -> t -> int -> int -> Unix.msg_flag list -> int
  = "lwt_unix_bytes_recv"
...
let recv fd buf pos len flags =
  if pos < 0 || len < 0 || pos > length buf - len then
    ...
    wrap_syscall Read fd (fun () ->
        stub_recv (unix_file_descr fd) buf pos len flags)
...
let send fd buf pos len flags =
  if pos < 0 || len < 0 || pos > length buf - len then
    ...
    wrap_syscall Write fd (fun () ->
        stub_send (unix_file_descr fd) buf pos len flags)

type io_vector = { iov_buffer : t; iov_offset : int; iov_length : int }

let io_vector ~buffer ~offset ~length : io_vector =
  { iov_buffer = buffer; iov_offset = offset; iov_length = length }
...
external stub_recvfrom :
  Unix.file_descr ->
  t ->
  int ->
  int ->
  Unix.msg_flag list ->
  int * Unix.sockaddr = "lwt_unix_bytes_recvfrom"
...
let recvfrom fd buf pos len flags =
  if pos < 0 || len < 0 || pos > length buf - len then
    ...
    wrap_syscall Read fd (fun () ->
        stub_recvfrom (unix_file_descr fd) buf pos len flags)

external stub_sendto :
  Unix.file_descr ->
  t ->
  int ->
  int ->
  Unix.msg_flag list ->
  Unix.sockaddr ->
  int = "lwt_unix_bytes_sendto_byte" "lwt_unix_bytes_sendto"
...
let sendto fd buf pos len flags addr =
  if pos < 0 || len < 0 || pos > length buf - len then
    ...
    wrap_syscall Write fd (fun () ->
        stub_sendto (unix_file_descr fd) buf pos len flags addr)
...
let madvise buf pos len advice =
  ...
  else stub_madvise buf pos len advice

external get_page_size : unit -> int = "lwt_unix_get_page_size"

let page_size = get_page_size ()

external stub_mincore : t -> int -> int -> bool array -> unit
  = "lwt_unix_mincore"
...
let mincore buffer offset states =
  ...
  then invalid_arg "Lwt_bytes.mincore"
  else stub_mincore buffer offset (Array.length states * page_size) states
...
let wait_mincore buffer offset =
  if offset < 0 || offset >= length buffer then
    ...
    if state.(0) then Lwt.return_unit
    else run_job (wait_mincore_job buffer offset)
