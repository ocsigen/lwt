...
(** A {b channel} is a high-level object for performing input/output (IO). It
    allows to read/write from/to the outside world in an efficient way, by
    minimising the number of system calls.

    An {b output channel} is used to send data and an {b input channel} is used
    to receive data.

    If you are familiar with buffered channels you may be familiar too with the
    {b flush} operation. Note that byte channels of this module are
    automatically flushed when there is nothing else to do (i.e. before the
    program becomes idle), so this means that you no longer have to write:
    ...
    Note about errors: input functions of this module raise [End_of_file] when
    the end-of-file is reached (i.e. when the read function returns [0]). Other
    exceptions are ones caused by the backend read/write functions, such as
    [Unix.Unix_error]. *)

exception Channel_closed of string
(** Exception raised when a channel is closed. The parameter is a description of
    the channel. *)
...
val make :
  ...
  'mode channel
(** [make ?buffer ?close ~mode perform_io] is the main function for creating new
    channels.

    @param buffer
      user-supplied buffer. When this argument is present, its value will be
      used as the buffer for the created channel. The size of buffer must
      conform to the limitations described in {!set_default_buffer_size}. When
      this argument is not present, a new internal buffer of default size will
      be allocated for this channel.

      Warning: do not use the same buffer for simultaneous work with more than
      one channel.

      There are other functions in this module that take a [buffer] argument,
      sharing the same semantics.
    @param close close function of the channel. It defaults to [Lwt.return]
    @param seek same meaning as [Unix.lseek]
    @param mode either {!input} or {!output}
    @param perform_io
      is the read or write function. It is called when more input is needed or
      when the buffer need to be flushed. *)

val of_bytes : mode:'mode mode -> Lwt_bytes.t -> 'mode channel
(** Create a channel from a byte array. Reading/writing is done directly on the
    provided array. *)
...
(** [close ch] closes the given channel. If [ch] is an output channel, it
    performs all pending actions, flushes it and closes it. If [ch] is an input
    channel, it just closes it immediately.

    [close] returns the result of the close function of the channel. Multiple
    calls to [close] will return exactly the same value.
...
(** [atomic f] transforms a sequence of io operations into one single atomic io
    operation.
...
(** [set_position ch pos] Sets the position in the output channel. This does not
    work if the channel does not support random access. *)
...
(** [read_line ic] reads one complete line from [ic] and returns it without the
    end of line. End of line is either ["\n"] or ["\r\n"].

    If the end of input is reached before reading any character, [End_of_file]
    is raised. If it is reached before reading an end of line but characters
    have already been read, they are returned. *)
...
(** [read_into ic buffer offset length] reads up to [length] bytes, stores them
    in [buffer] at offset [offset], and returns the number of bytes read.

    Note: [read_into] does not raise [End_of_file], it returns a length of [0]
    instead. *)

val read_into_exactly : input_channel -> bytes -> int -> int -> unit Lwt.t
(** [read_into_exactly ic buffer offset length] reads exactly [length] bytes and
    stores them in [buffer] at offset [offset].

    @raise End_of_file on end of input *)

val read_into_bigstring :
  input_channel -> Lwt_bytes.t -> int -> int -> int Lwt.t

val read_into_exactly_bigstring :
  input_channel -> Lwt_bytes.t -> int -> int -> unit Lwt.t
...
(** [read_value channel] reads a marshaled value from [channel]; it corresponds
    ...
    {{:https://ocaml.org/api/Marshal.html#VALfrom_channel}
    [Marshal.from_channel]}. The corresponding writing function is
    {!write_value}.

    Note that reading marshaled values is {e not}, in general, type-safe. See
    the warning in the description of module
    {{:https://ocaml.org/api/Marshal.html} [Marshal]} for details. The short
    version is: if you read a value of one type, such as [string], when a value
    of another type, such as [int] has actually been marshaled to [channel], you
    may get arbitrary behavior, including segmentation faults, access
    violations, security bugs, etc. *)

(** {2 Writing} *)

(** Note: as for reading functions, all functions except {!write_chars} and
    {!write_lines} are {b atomic}.

    For example if you use {!write_line} in two different threads, the two
    operations will be serialized, and lines cannot be mixed. *)
...
(** [write_from oc buffer offset length] writes up to [length] bytes to [oc],
    from [buffer] at offset [offset] and returns the number of bytes actually
    written *)

val write_from_bigstring :
  output_channel -> Lwt_bytes.t -> int -> int -> int Lwt.t
...
(** [write_from_exactly oc buffer offset length] writes all [length] bytes from
    [buffer] at offset [offset] to [oc] *)

val write_from_exactly_bigstring :
  output_channel -> Lwt_bytes.t -> int -> int -> unit Lwt.t
...
(** [write_value channel ?flags v] writes [v] to [channel] using the [Marshal]
    ...
    {{:https://ocaml.org/api/Marshal.html#VALto_channel} [Marshal.to_channel]}
...
(** These functions are basically helpers. Also you may prefer using the name
    {!printl} rather than {!write_line} because it is shorter.
...
(** [Lwt_io.open_file ~mode file] opens the given file, either for reading (with
    ...
    {{:https://ocaml.org/api/Unix.html#TYPEopen_flag} [Unix.open_flag]}). Note
    that [~flags] is used {e exactly} as given. For example, opening a file with
    [~flags] and [~mode:Input] does {e not} implicitly add [O_RDONLY]. So, you
    should include [O_RDONLY] when opening for reading ([~mode:Input]), and
    [O_WRONLY] when opening for writing ([~mode:Input]). It is also recommended
    to include [O_NONBLOCK], unless you are sure that the file cannot be a
...
(** [open_temp_file ()] starts creating a new temporary file, and evaluates to a
    ...
    flags are used exactly. Note that these should typically contain at least
...
(** [open_connection ?fd ?in_buffer ?out_buffer addr] opens a connection to the
    given address and returns two channels for using it. If [fd] is not
...
(** [chars_of_file name] returns a stream of all characters of the file with
    name [name]. As for {!lines_of_file} the file is closed when all characters
    have been read. *)
...
(** [block ch size f] pass to [f] the internal buffer and an offset. The buffer
    contains [size] chars at [offset]. [f] may read or write these chars. [size]
    must satisfy [0 <= size <= 16] *)
...
type direct_access = {
  ...
  da_perform : unit -> int Lwt.t;
      (** - for input channels: refills the buffer and returns how many bytes
            have been read
          - for output channels: flush partially the buffer and returns how many
...
(** [direct_access ch f] passes to [f] a {!direct_access} structure. [f] must
    use it and update [da_ptr] to reflect how many bytes have been read/written. *)

(** {2 Misc} *)

val default_buffer_size : unit -> int
(** Return the default size for buffers. Channels that are created without a
    specific buffer use new buffer of this size. *)

val set_default_buffer_size : int -> unit
(** Change the default buffer size.

    @raise Invalid_argument
      if the given size is smaller than [16] or greater than
