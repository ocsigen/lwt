...
let shell =
  if Sys.win32 then fun cmd -> ("", [| "cmd.exe"; "/c"; "\000" ^ cmd |])
  else fun cmd -> ("", [| "/bin/sh"; "-c"; cmd |])
...
type proc = {
  id : int;
  (* The process id. *)
  fd : Unix.file_descr; (* A handle on windows, and a dummy value of Unix. *)
}

let win32_get_fd fd redirection =
  match redirection with
  | `Keep -> Some fd
  | `Dev_null -> Some (Unix.openfile "nul" [ Unix.O_RDWR ] 0o666)
  | `Close -> None
  | `FD_copy fd' -> Some fd'
  | `FD_move fd' -> Some fd'
...
let win32_spawn (prog, args) env ?cwd ?(stdin : redirection = `Keep)
    ?(stdout : redirection = `Keep) ?(stderr : redirection = `Keep) toclose =
  ...
  let proc =
    win32_create_process
      (if prog = "" then None else Some prog)
      cmdline env cwd
  ...
  let close fd fd' =
    ...
    | `Dev_null -> (
        match fd' with Some fd' -> Unix.close fd' | None -> assert false)
    | _ -> ()
  in
...
let win32_terminate proc = win32_terminate_process proc.fd 1

let unix_redirect fd redirection =
  match redirection with
  | `Keep -> ()
  | `Dev_null ->
      Unix.close fd;
...
let unix_spawn (prog, args) env ?cwd ?(stdin : redirection = `Keep)
    ?(stdout : redirection = `Keep) ?(stderr : redirection = `Keep) toclose =
  ...
  | 0 -> (
      ...
      with _ -> (* Do not run at_exit hooks *)
                unix_exit 127)
  | id ->
      let close = function `FD_move fd -> Unix.close fd | _ -> () in
      close stdin;
      close stdout;
      close stderr;
      { id; fd = Unix.stdin }
...
let unix_terminate proc = Unix.kill proc.id Sys.sigkill
...
class virtual common timeout proc channels =
  ...
  object (self)
    ...
    method terminate = if Lwt.state wait = Lwt.Sleep then terminate proc

    method close =
      if closed then self#status
      else (
        closed <- true;
        Lwt.protected (Lwt.join (List.map Lwt_io.close channels)) >>= fun () ->
        self#status)
          ...
          ((* Ignore errors since they can be obtained by
           ...
           Lwt.try_bind
             (fun () ->
               Lwt.choose
                 [
                   (Lwt_unix.sleep dt >>= fun () -> Lwt.return_false);
...
class process_in ?timeout ?env ?cwd ?stdin ?stderr cmd =
  ...
  let proc =
    spawn cmd env ?cwd ?stdin ~stdout:(`FD_move stdout_w) ?stderr [ stdout_r ]
  in
...
class process_out ?timeout ?env ?cwd ?stdout ?stderr cmd =
  ...
  let proc =
    spawn cmd env ?cwd ~stdin:(`FD_move stdin_r) ?stdout ?stderr [ stdin_w ]
  in
...
class process ?timeout ?env ?cwd ?stderr cmd =
  let stdin_r, stdin_w = Unix.pipe () and stdout_r, stdout_w = Unix.pipe () in
  let proc =
    spawn cmd env ?cwd ~stdin:(`FD_move stdin_r) ~stdout:(`FD_move stdout_w)
      ?stderr [ stdin_w; stdout_r ]
  in
...
class process_full ?timeout ?env ?cwd cmd =
  ...
  object
    inherit
      common
        timeout proc
        [ cast_chan stdin; cast_chan stdout; cast_chan stderr ]
...
let make_with backend ?timeout ?env ?cwd cmd f =
  ...
  Lwt.finalize
    (fun () -> f process)
    (fun () -> process#close >>= fun _ -> Lwt.return_unit)
...
let ignore_close ch = ignore (Lwt_io.close ch)

let read_opt read ic =
  (Lwt.catch
     (fun () -> read ic >|= fun x -> Some x)
     (function
       | Unix.Unix_error (Unix.EPIPE, _, _) | End_of_file -> Lwt.return_none
...
let recv_chars pr =
  ...
  Lwt_stream.from (fun _ ->
      ...
      if x = None then Lwt_io.close ic >>= fun () -> Lwt.return x
      else Lwt.return x)
...
let recv_line pr =
  let ic = pr#stdout in
  Lwt.finalize (fun () -> Lwt_io.read_line ic) (fun () -> Lwt_io.close ic)
...
let pwrite_chars ?timeout ?env ?cwd ?stdout ?stderr cmd chars =
  send Lwt_io.write_chars
    (open_process_out ?timeout ?env ?cwd ?stdout ?stderr cmd)
    chars

let pwrite_line ?timeout ?env ?cwd ?stdout ?stderr cmd line =
  send Lwt_io.write_line
    (open_process_out ?timeout ?env ?cwd ?stdout ?stderr cmd)
    line

let pwrite_lines ?timeout ?env ?cwd ?stdout ?stderr cmd lines =
  send Lwt_io.write_lines
    (open_process_out ?timeout ?env ?cwd ?stdout ?stderr cmd)
    lines

(* Mapping *)

type 'a map_state = Init | Save of 'a option Lwt.t | Done
