...
external cast_node : 'a Lwt_sequence.node -> Obj.t Lwt_sequence.node
  = "%identity"
...
class virtual abstract =
  object (self)
    ...
    method destroy =
      ...
      Lwt_sequence.iter_l
        (fun (_delay, _repeat, _f, _g, ev) -> stop_event ev)
        timers;
      self#cleanup
    ...
    method transfer (engine : abstract) =
      Lwt_sequence.iter_l
        (fun (fd, f, _g, ev) ->
          stop_event ev;
          ev := !(engine#on_readable fd f))
        readables;
      Lwt_sequence.iter_l
        (fun (fd, f, _g, ev) ->
          stop_event ev;
          ev := !(engine#on_writable fd f))
        writables;
      Lwt_sequence.iter_l
        (fun (delay, repeat, f, _g, ev) ->
          stop_event ev;
          ev := !(engine#on_timer delay repeat f))
        timers

    method fake_io fd =
      Lwt_sequence.iter_l
        (fun (fd', _f, g, _stop) -> if fd = fd' then g ())
        readables;
      Lwt_sequence.iter_l
        (fun (fd', _f, g, _stop) -> if fd = fd' then g ())
        writables
    ...
    method on_readable fd f =
      ...
      ev :=
        { stop; node = cast_node (Lwt_sequence.add_r (fd, f, g, ev) readables) };
      ev
    ...
    method on_writable fd f =
      ...
      ev :=
        { stop; node = cast_node (Lwt_sequence.add_r (fd, f, g, ev) writables) };
      ev
    ...
    method on_timer delay repeat f =
      ...
      ev :=
        {
          stop;
          node = cast_node (Lwt_sequence.add_r (delay, repeat, f, g, ev) timers);
        };
      ev
...
external ev_readable_init :
  ev_loop -> Unix.file_descr -> (unit -> unit) -> ev_io
  = "lwt_libev_readable_init"

external ev_writable_init :
  ev_loop -> Unix.file_descr -> (unit -> unit) -> ev_io
  = "lwt_libev_writable_init"

external ev_io_stop : ev_loop -> ev_io -> unit = "lwt_libev_io_stop"

external ev_timer_init : ev_loop -> float -> bool -> (unit -> unit) -> ev_timer
  = "lwt_libev_timer_init"

external ev_timer_stop : ev_loop -> ev_timer -> unit = "lwt_libev_timer_stop"

class libev ?(backend = Ev_backend.default) () =
  object
    inherit abstract
...
type sleeper = {
  mutable time : float; (* The time at which the sleeper should be wakeup. *)
  mutable stopped : bool; (* [true] iff the event has been stopped. *)
  action : unit -> unit; (* The action for the sleeper. *)
}

module Sleep_queue = Lwt_pqueue.Make (struct
  type t = sleeper
  ...
  object
    ...
    method private register_timer delay repeat f =
      if repeat then (
        ...
        let sleeper =
          { time = Unix.gettimeofday () +. delay; stopped = false; action = f }
        in
        new_sleeps <- sleeper :: new_sleeps;
        lazy (sleeper.stopped <- true)
  ...
  object (self)
    ...
    method iter block =
      (* Transfer all sleepers added since the last iteration to the
         main sleep queue: *)
      sleep_queue <-
        List.fold_left (fun q e -> Sleep_queue.add e q) sleep_queue new_sleeps;
      new_sleeps <- [];
      ...
      let fds_r, fds_w =
        try self#select fds_r fds_w timeout with
...
class virtual poll_based =
  object (self)
    ...
    method iter block =
      ...
      let fds = [] in
      let fds =
        Fd_map.fold (fun fd _ l -> (fd, true, false) :: l) wait_readable fds
      in
      let fds =
        Fd_map.fold (fun fd _ l -> (fd, false, true) :: l) wait_writable fds
      in
      (* Compute the timeout. *)
      let timeout = if block then get_next_timeout sleep_queue else 0. in
      (* Do the blocking call *)
      let fds =
        try self#poll fds timeout with
        | Unix.Unix_error (Unix.EINTR, _, _) -> []
        | Unix.Unix_error (Unix.EBADF, _, _) ->
            (* Keeps only bad file descriptors. Actions registered on
               them have to handle the error: *)
