...
(** This module bridges the gap between {{:https://ocaml.org/api/Format.html}
    [Format]} and {!Lwt}. Although it is not required, it is recommended to use
    this module with the {{:http://erratique.ch/software/fmt} [Fmt]} library.
...
type order =
  | String of string * int * int
      (** [String (s, off, len)] indicate the output of [s] at offset [off] and
          length [len]. *)
  | Flush  (** Flush operation *)

val make_stream : unit -> order Lwt_stream.t * formatter
(** [make_stream ()] returns a formatter and a stream of all the writing order
    given on that stream. *)
...
(** [make_formatter ~commit ~fmt] creates a new lwt formatter based on the
    {{:https://ocaml.org/api/Format.html#TYPEformatter} [Format.formatter]}
...
(** [get_formatter fmt] returns the underlying
    {{:https://ocaml.org/api/Format.html#TYPEformatter} [Format.formatter]}. To
    access the underlying formatter during printing, it isvrecommended to use
    [%t] and [%a]. *)

(** {2 Printing} *)

val fprintf :
  formatter -> ('a, Format.formatter, unit, unit Lwt.t) format4 -> 'a
...
(** [flush fmt] flushes the formatter (as with
    ...
    [Format.pp_print_flush]}) and executes all the printing action on the
    underlying channel. *)
...
(** Write all the pending orders of a formatter. Warning: This function flush
    neither the internal format queues nor the underlying channel and is
    intended for low level use only. You should probably use {!flush} instead. *)
