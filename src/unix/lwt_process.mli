...
(** A command. The first field is the name of the executable and the second is
    the list of arguments. For example:

    {[ "ls", [| "ls"; "-l" |] ]}

    Notes:

    - if the name is the empty string, then the first argument will be used. You
      should specify a name only if you do not want the executable to be
      searched in the PATH. On Windows the only way to enable automatic search
      in PATH is to pass an empty name.

    - it is possible to ``inline'' an argument, i.e. split it into multiple
      arguments. To do that prefix it with ["\000"]. For example:
...
(** A command executed with the shell. (with ["/bin/sh -c <cmd>"] on Unix and
    ["cmd.exe /c <cmd>"] on Windows). *)

(** All the following functions take an optional argument [timeout]. If
    specified, after expiration, the process will be sent a [Unix.sigkill]
    signal and channels will be closed. When the channels are closed, any
    pending I/O operations on them (such as {!Lwt_io.read_chars}) fail with
    exception {!Lwt_io.Channel_closed}. *)
...
class process_none :
  ...
  -> object
       ...
       (** Terminates the process. It is equivalent to [kill Sys.sigkill] on
           Unix but also works on Windows (unlike {!kill}). *)

       method status : Unix.process_status Lwt.t
       (** Threads which wait for the sub-process to exit then returns its exit
           status *)
...
val open_process_full :
  ?timeout:float -> ?env:string array -> ?cwd:string -> command -> process_full
