...
(** Lwt_unix feature discovery script.
    ...
    {v       dune exec src/unix/config/discover.exe -- --help v}
    ...
    do should be clear from the feature tests that are already in that module. *)
...
module C_library_flags : sig
  ...
  let default_search_paths =
    [ "/usr"; "/usr/local"; "/usr/pkg"; "/opt"; "/opt/local"; "/sw"; "/mingw" ]
