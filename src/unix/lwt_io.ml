...
(* A channel state *)
type 'mode state =
  | Busy_primitive (* A primitive is running on the channel *)
  | Busy_atomic of 'mode channel (* An atomic operations is being performed on the channel. The
                                    argument is the temporary atomic wrapper. *)
  | Waiting_for_busy (* A queued operation has not yet started. *)
  | Idle (* The channel is unused *)
...
and 'mode channel = {
  ...
  channel : 'mode _channel; (* The real channel *)
  mutable queued : unit Lwt.u Lwt_sequence.t; (* Queued operations *)
}

and 'mode _channel = {
  mutable buffer : Lwt_bytes.t;
  mutable length : int;
  mutable ptr : int; (* Current position *)
  mutable max : int;
  ...
  main : 'mode channel; (* The main wrapper *)
  close : unit Lwt.t Lazy.t; (* Close function *)
  mode : 'mode mode; (* The channel mode *)
  mutable offset : int64; (* Number of bytes really read/written *)
  typ : typ; (* Type of the channel. *)
}
...
let position : type mode. mode channel -> int64 =
 fun wrapper ->
  let ch = wrapper.channel in
  match ch.mode with
  | Input -> Int64.sub ch.offset (Int64.of_int (ch.max - ch.ptr))
  | Output -> Int64.add ch.offset (Int64.of_int ch.ptr)

let name : type mode. mode _channel -> string =
 fun ch -> match ch.mode with Input -> "input" | Output -> "output"
...
let is_busy ch =
  match ch.state with
  | Invalid -> raise (invalid_channel ch.channel)
  | Idle | Closed -> false
  | Busy_primitive | Busy_atomic _ | Waiting_for_busy -> true

(* Flush/refill the buffer. No race condition could happen because
   this function is always called atomically: *)
let perform_io : type mode. mode _channel -> int Lwt.t =
 fun ch ->
  match ch.main.state with
  | Closed -> Lwt.fail (closed_channel ch)
  | Invalid -> Lwt.fail (invalid_channel ch)
  | Idle | Waiting_for_busy -> assert false
...
let rec flush_total oc =
  if oc.ptr > 0 then flush_partial oc >>= fun _ -> flush_total oc
  else Lwt.return_unit

let safe_flush_total oc =
  Lwt.catch (fun () -> flush_total oc) (fun _ -> Lwt.return_unit)

let deepest_wrapper ch =
  let rec loop wrapper =
    match wrapper.state with
    | Busy_atomic wrapper -> loop wrapper
    | Busy_primitive | Waiting_for_busy | Idle | Closed | Invalid -> wrapper
  in
  loop ch.main
...
let auto_flush oc =
  ...
  | Idle ->
      ...
      if wrapper.state = Busy_primitive then wrapper.state <- Idle;
      if not (Lwt_sequence.is_empty wrapper.queued) then
        Lwt.wakeup_later (Lwt_sequence.take_l wrapper.queued) ();
      Lwt.return_unit
  | Closed | Invalid -> Lwt.return_unit

(* A ``locked'' channel is a channel in the state [Busy_primitive] or
   [Busy_atomic] *)

let unlock : type m. m channel -> unit =
 fun wrapper ->
  match wrapper.state with
  | Busy_primitive | Busy_atomic _ ->
      if Lwt_sequence.is_empty wrapper.queued then wrapper.state <- Idle
      else (
      ...
      let ch = wrapper.channel in
      if
        (* Launch the auto-flusher only if the channel is not busy: *)
        wrapper.state = Idle
        ...
        Lwt_sequence.is_empty wrapper.queued
      then (
      ...
      | Idle | Waiting_for_busy ->
          ...
          Lwt.finalize
            ...
            (fun () ->
              unlock wrapper;
              Lwt.return_unit)
      | Invalid -> Lwt.fail (invalid_channel wrapper.channel)
      | Busy_primitive | Busy_atomic _ -> assert false)
  | Closed -> Lwt.fail (closed_channel wrapper.channel)
  | Invalid -> Lwt.fail (invalid_channel wrapper.channel)
...
let rec abort wrapper =
  ...
  | Idle | Busy_primitive | Waiting_for_busy ->
      ...
      (* Abort any current real reading/writing operation on the
         channel: *)
      Lwt.wakeup_exn wrapper.channel.abort_wakener
        (closed_channel wrapper.channel);
      Lazy.force wrapper.channel.close

let close : type mode. mode channel -> unit Lwt.t =
 fun wrapper ->
  let channel = wrapper.channel in
  if channel.main != wrapper then
    Lwt.fail
      (Failure "Lwt_io.close: cannot close a channel obtained via Lwt_io.atomic")
  else
    ...
    | Output ->
        Lwt.catch
          (fun () ->
            ...
            primitive
              (fun channel ->
                safe_flush_total channel >>= fun () -> abort wrapper)
              wrapper)
          (fun _ -> abort wrapper)
...
let make :
 ...
 fun ?buffer ?(close = Lwt.return) ?(seek = no_seek) ~mode perform_io ->
  ...
  let rec ch =
    {
      ...
      length = size;
      ptr = 0;
      max = (match mode with Input -> 0 | Output -> size);
  ...
  and wrapper =
    { state = Idle; channel = ch; queued = Lwt_sequence.create () }
  in
  (match mode with Input -> () | Output -> Outputs.add outputs wrapper);
  wrapper
  ...
  let rec ch =
    {
      ...
      auto_flushing = true;
      mode;
      offset = (match mode with Output -> 0L | Input -> Int64.of_int length);
      typ = Type_bytes;
    }
  and wrapper =
    { state = Idle; channel = ch; queued = Lwt_sequence.create () }
...
let of_fd :
 ...
 fun ?buffer ?close ~mode fd ->
  let perform_io =
    match mode with Input -> Lwt_bytes.read fd | Output -> Lwt_bytes.write fd
  in
  make ?buffer
    ~close:
      (match close with Some f -> f | None -> fun () -> Lwt_unix.close fd)
...
let buffered : type m. m channel -> int =
 fun ch ->
...
let resize_buffer : type m. m channel -> int -> unit Lwt.t =
 fun wrapper len ->
  ...
  | Type_normal _ ->
      let f : type m. m _channel -> unit Lwt.t =
       fun ch ->
        match ch.mode with
        | Input ->
            ...
            if len < unread_count then
              Lwt.fail
                (Failure
                   ("Lwt_io.resize_buffer: cannot decrease buffer size, too \
                     much "
                   ^ "unread data"))
            else
        ...
        | Output ->
            ...
            let rec loop () =
              if ch.ptr > len then flush_partial ch >>= fun _ -> loop ()
              else Lwt.return_unit
            in
            loop () >>= fun () ->
            let buffer = Lwt_bytes.create len in
...
module Primitives = struct
  ...
  let rec read_char ic =
    let ptr = ic.ptr in
    if ptr = ic.max then
      refill ic >>= function 0 -> Lwt.fail End_of_file | _ -> read_char ic
    else (
      ic.ptr <- ptr + 1;
  ...
  let unsafe_read_into_exactly' read_into ic buf ofs len =
    let rec loop ic buf ofs len =
      ...
      | n ->
          let len = len - n in
          if len = 0 then Lwt.return_unit else loop ic buf (ofs + n) len
    in
    loop ic buf ofs len
  ...
  let read_into_exactly_bigstring ic buf ofs len =
    ...
    else if len = 0 then Lwt.return_unit
    else unsafe_read_into_exactly_bigstring ic buf ofs len

  let read_into_exactly ic buf ofs len =
    if ofs < 0 || len < 0 || ofs + len > Bytes.length buf then
      Lwt.fail (Invalid_argument "Lwt_io.read_into_exactly")
    else if len = 0 then Lwt.return_unit
    else unsafe_read_into_exactly ic buf ofs len
  ...
  let rec read_all ic total_len acc =
    ...
    ic.ptr <- ic.max;
    refill ic >>= function
    | 0 -> Lwt.return (rev_concat (len + total_len) (str :: acc))
    | _ -> read_all ic (len + total_len) (str :: acc)

  let read count ic =
    match count with
    | None -> read_all ic 0 [] >|= Bytes.unsafe_to_string
    | Some len ->
        let buf = Bytes.create len in
        unsafe_read_into ic buf 0 len >>= fun real_len ->
        if real_len < len then
          Lwt.return Bytes.(sub buf 0 real_len |> unsafe_to_string)
        else Lwt.return (Bytes.unsafe_to_string buf)
  ...
  let rec write_char oc ch =
    let ptr = oc.ptr in
    if ptr < oc.length then (
      oc.ptr <- ptr + 1;
      Lwt_bytes.unsafe_set oc.buffer ptr ch;
      Lwt.return_unit)
    else flush_partial oc >>= fun _ -> write_char oc ch
  ...
  let write_value oc ?(flags = []) x = write oc (Marshal.to_string x flags)
  ...
  let rec write_block_unsafe oc size f =
    if oc.max - oc.ptr < size then
      flush_partial oc >>= fun _ -> write_block_unsafe oc size f
    else
      let ptr = oc.ptr in
      oc.ptr <- ptr + size;
      f oc.buffer ptr
  ...
  let block :
      type m. m _channel -> int -> (Lwt_bytes.t -> int -> 'a Lwt.t) -> 'a Lwt.t
      =
   fun ch size f ->
    if size < 0 || size > min_buffer_size then
      Lwt.fail (Invalid_argument "Lwt_io.block")
    else if ch.max - ch.ptr >= size then (
      let ptr = ch.ptr in
      ch.ptr <- ptr + size;
      f ch.buffer ptr)
    else
      match ch.mode with
      | Input -> read_block_unsafe ch size f
      | Output -> write_block_unsafe ch size f
  ...
  module MakeNumberIO (Endian : EndianBigstring.EndianBigstringSig) = struct
    (* +-------------------------------------------------------------+
       | Reading numbers                                             |
       +-------------------------------------------------------------+ *)

    let read_int ic =
      read_block_unsafe ic 4 (fun buffer ptr ->
          Lwt.return (Int32.to_int (Endian.get_int32 buffer ptr)))

    let read_int16 ic =
      read_block_unsafe ic 2 (fun buffer ptr ->
    ...
    let read_int64 ic =
      read_block_unsafe ic 8 (fun buffer ptr ->
    ...
    let write_int oc v =
      write_block_unsafe oc 4 (fun buffer ptr ->
          Endian.set_int32 buffer ptr (Int32.of_int v);
          Lwt.return_unit)

    let write_int16 oc v =
      write_block_unsafe oc 2 (fun buffer ptr ->
    ...
    let write_int64 oc v =
      write_block_unsafe oc 8 (fun buffer ptr ->
  ...
  let set_position : type m. m _channel -> int64 -> unit Lwt.t =
   fun ch pos ->
    ...
    | Type_normal (_, seek), Input ->
        ...
        if pos >= current && pos <= ch.offset then (
  ...
  let length ch =
    ...
    | Type_normal (_, seek) ->
        ...
        do_seek "length" seek ch.offset >>= fun () -> Lwt.return len
    | Type_bytes -> Lwt.return (Int64.of_int ch.length)
end
...
let read_char wrapper =
  ...
  if wrapper.state = Idle && ptr < channel.max then (
    channel.ptr <- ptr + 1;
    Lwt.return (Lwt_bytes.unsafe_get channel.buffer ptr))
  else primitive Primitives.read_char wrapper
...
let read_char_opt wrapper =
  ...
  else primitive Primitives.read_char_opt wrapper
...
let read_into_exactly_bigstring ic bytes ofs len =
  primitive
    (fun ic -> Primitives.read_into_exactly_bigstring ic bytes ofs len)
    ic
...
let write_char wrapper x =
  ...
  if wrapper.state = Idle && ptr < channel.max then (
    ...
    if not channel.auto_flushing then (
      channel.auto_flushing <- true;
      ignore (auto_flush channel);
      Lwt.return_unit)
    else Lwt.return_unit)
  else primitive (fun oc -> Primitives.write_char oc x) wrapper
...
let write_from_exactly_bigstring oc bytes ofs len =
  primitive
    (fun oc -> Primitives.write_from_exactly_bigstring oc bytes ofs len)
    oc
...
let zero =
  make ~mode:input ~buffer:(Lwt_bytes.create min_buffer_size)
    (fun str ofs len ->
      Lwt_bytes.fill str ofs len '\x00';
      Lwt.return len)

let null =
  make ~mode:output ~buffer:(Lwt_bytes.create min_buffer_size)
...
let open_file :
 ...
 fun ?buffer ?flags ?perm ~mode filename ->
  let flags =
    ...
    | Some l, _ -> l
    | None, Input -> [ Unix.O_RDONLY; Unix.O_NONBLOCK ]
    | None, Output ->
        [ Unix.O_WRONLY; Unix.O_CREAT; Unix.O_TRUNC; Unix.O_NONBLOCK ]
  and perm =
    match (perm, mode) with
    | Some p, _ -> p
    | None, Input -> 0
    | None, Output -> 0o666
  in
...
let with_file ?buffer ?flags ?perm ~mode filename f =
  ...
  Lwt.finalize (fun () -> f ic) (fun () -> close ic)
...
let open_temp_file ?buffer ?flags ?perm ?temp_dir ?prefix ?(suffix = "") () =
  ...
  in
  let prefix =
    match prefix with None -> "lwt_io_temp_file_" | Some prefix -> prefix
  in
...
let with_temp_file ?buffer ?flags ?perm ?temp_dir ?prefix ?suffix f =
  open_temp_file ?buffer ?flags ?perm ?temp_dir ?prefix ?suffix ()
  >>= fun (fname, chan) ->
  Lwt.finalize
    (fun () -> f (fname, chan))
    (fun () -> close chan >>= fun () -> Lwt_unix.unlink fname)

let create_temp_dir ?(perm = 0o755) ?(parent = Filename.get_temp_dir_name ())
    ?(prefix = "lwt_io_temp_dir_") ?(suffix = "") () =
  let rec attempt n =
    let name = temp_file_name parent prefix suffix in
    Lwt.catch
      (fun () -> Lwt_unix.mkdir name perm >>= fun () -> Lwt.return name)
      (function
...
let rec delete_recursively directory =
  ...
  |> Lwt_stream.iter_s (fun entry ->
         if
           entry = Filename.current_dir_name || entry = Filename.parent_dir_name
         then Lwt.return ()
         else
...
let with_close_connection f (ic, oc) =
  ...
  let close_if_not_closed channel =
    if is_closed channel then Lwt.return_unit else close channel
  in
...
let establish_server_generic bind_function ?fd:preexisting_socket_for_listening
    ?(backlog = (Lwt_unix.somaxconn () [@ocaml.warning "-3"])) listening_address
  ...
  let rec accept_loop () =
    let try_to_accept =
      Lwt.catch
        (fun () -> Lwt_unix.accept listening_socket >|= fun x -> `Accepted x)
        (function
          | Unix.Unix_error (Unix.ECONNABORTED, _, _) -> Lwt.return `Try_again
          | e -> Lwt.fail e)
    in
    ...
    | `Should_stop ->
        ...
        (match[@ocaml.warning "-4"] listening_address with
...
let establish_server_with_client_socket ?server_fd ?backlog ?(no_close = false)
    sockaddr f =
  ...
  let server, server_started =
    establish_server_generic Lwt_unix.bind ?fd:server_fd ?backlog sockaddr
      handler
  in
  server_started >>= fun () -> Lwt.return server

let establish_server_with_client_address_generic bind_function ?fd
    ?(buffer_size = !default_buffer_size) ?backlog ?(no_close = false) sockaddr
    handler =
  let best_effort_close channel =
    ...
    if is_closed channel then Lwt.return_unit
    else
      Lwt.catch
        (fun () -> close channel)
        (fun exn ->
  ...
  let handler client_address client_socket =
    Lwt.async (fun () ->
        ...
        (* Not using Lwt.finalize here, to make sure that exceptions from [f]
           ...
           channels. *)
        Lwt.catch
          (fun () -> handler client_address (input_channel, output_channel))
          (fun exn ->
            !Lwt.async_exception_hook exn;
            Lwt.return_unit)
        >>= fun () ->
...
let establish_server_with_client_address ?fd ?buffer_size ?backlog ?no_close
    sockaddr handler =
  let server, server_started =
    establish_server_with_client_address_generic Lwt_unix.bind ?fd ?buffer_size
      ?backlog ?no_close sockaddr handler
  in
  server_started >>= fun () -> Lwt.return server

let establish_server ?fd ?buffer_size ?backlog ?no_close sockaddr f =
  let f _addr c = f c in
  establish_server_with_client_address ?fd ?buffer_size ?backlog ?no_close
    sockaddr f
...
let establish_server_deprecated ?fd ?buffer_size ?backlog sockaddr f =
  ...
  let server, server_started =
    establish_server_with_client_address_generic blocking_bind ?fd ?buffer_size
      ?backlog ~no_close:true sockaddr f
  in
...
let ignore_close ch = ignore (close ch)
...
let make_stream f lazy_ic =
  ...
  Lwt_stream.from (fun _ ->
      Lazy.force lazy_ic >>= fun ic ->
      f ic >>= fun x ->
      if x = None then close ic >>= fun () -> Lwt.return x else Lwt.return x)
