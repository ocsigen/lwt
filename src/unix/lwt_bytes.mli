...
(** [get buffer offset value] changes the value of the byte at offset [offset]
    in [buffer] to [value]. *)
...
(** [of_bytes buf] returns a newly allocated byte array with the same contents
    as [buf]. *)

val of_string : string -> t
(** [of_string buf] returns a newly allocated byte array with the same contents
    as [buf]. *)
...
(** [to_string buf] returns a newly allocated string with the same contents as
    [buf]. *)

(** {2 Copying} *)

val blit : t -> int -> t -> int -> int -> unit
(** [blit buf1 ofs1 buf2 ofs2 len] copies [len] bytes from [buf1] starting at
    offset [ofs1] to [buf2] starting at offset [ofs2]. *)
...
(** [proxy buffer offset length] creates a ``proxy''. The returned byte array
    share the data of [buffer] but with different bounds. *)

val extract : t -> int -> int -> t
(** [extract buffer offset length] creates a new byte array of length [length]
    and copy the [length] bytes of [buffer] at [offset] into it. *)
...
(** The following functions behave similarly to the ones in {!Lwt_unix}, except
    they use byte arrays instead of [Bytes.t], and they never perform extra
    copies of the data. *)
...
val recv :
  Lwt_unix.file_descr -> t -> int -> int -> Unix.msg_flag list -> int Lwt.t
...
val map_file :
  fd:Unix.file_descr -> ?pos:int64 -> shared:bool -> ?size:int -> unit -> t
...
(** Type of advise that can be sent to the kernel by the program. See the manual
    madvise(2) for a description of each. *)
type advice =
  | MADV_NORMAL
  | MADV_RANDOM
  | MADV_SEQUENTIAL
...
(** [madvise buffer pos len advice] advises the kernel how the program will use
    the memory mapped file between [pos] and [pos + len].
...
(** [mincore buffer offset states] tests whether the given pages are in the
    ...
    {!page_size}. [states] is used to store the result; each cases is [true] if
    the corresponding page is in RAM and [false] otherwise.
