...
and +'a node = 
  | Nil
  | Cons of 'a * 'a t (* A fully-evaluated list node, either empty or containing an element and a delayed tail. *)
```
```ocaml
val empty : 'a t
```
...
See [`map_s`](./#val-map_s) for additional details about scheduling.
...
The sequence's next node is evaluated only once the function has finished processing the current element. More formally: the promise for the `n+1`th node of the sequence is created only once the promise returned by `f` on the `n`th element of the sequence has resolved.
